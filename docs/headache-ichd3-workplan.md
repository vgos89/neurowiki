...
**Last updated:** 2026-07-06 · after run 2 (A-M3 + A-m4 shipped, clinical-reviewer approved)
...
| A-M1 | Vestibular migraine: add ≥5-episode, 5min–72h, mod/severe, ≥50%-of-episodes gates | A1.6.6 | **material** | ⏳ pending | over-calls VM; **blocked on source confirmation** — appendix criteria not yet read from the PDF |
| A-m6 | Hemicrania continua: add "aggravation by movement" alternative to criterion C | §3.4 C.2 | minor | 📋 planned | `hc-C` accepts only restlessness/autonomic; add movement-aggravation chip |
