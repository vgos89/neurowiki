...
1. Trial arm enrichment (commit c1146eb): control/comparator arms on 5 pilot trials (NINDS, ECASS III, ESCAPE, DEFUSE-3, DAWN) are now PDF-sourced and substantive. `armDetails[]` on each trial record transcribed character-for-character from publication PDFs (primary papers as source-of-truth over registries). Study Arms accordion repositioned directly under Primary Outcome (single render per page, cross-archetype consistency). NINDS eligibility full-text verified → High confidence. ESCAPE primary OR 2.6 confirmed correct (pre-specified unadjusted primary; label clarification pending owner sign-off, no value change). Remaining ~74 NCT-linked trials are candidates for backfill in future reviewed waves. Schema is stable, components reusable.
