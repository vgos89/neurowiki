...
## Follow-up review — 2026-06-10 (three-tier window chip + citation conflation)

The window chip was expanded from two states to three tiers (green "Within 4.5h" ≤4.5h, orange "4.5–9h window" 4.5–9h, red "Beyond 9h" >9h). The first review of this expansion **BLOCKED** it: the orange 4.5–9h tier cited `aha-asa-2026-4.6.3`, whose stored quote describes a different recommendation (late-window tenecteplase for LVO, 4.5–24h, Class IIb), not the 4.5–9h perfusion-selected extended IV thrombolysis window. The block also surfaced a pre-existing registry conflation: §4.6.3 was titled "Extended-window IV thrombolysis" but its quote and four dependent claims were the late-window-TNK recommendation, while one live-page card (`late-window-selection-guideline-summary`) and the chip borrowed §4.6.3 for the 4.5–9h window.

**Resolution (V-approved 2026-06-10):**
- Added foundational trial citations `extend-trial-2019` (EXTEND, PMID 31067369) and `wake-up-trial-2018` (WAKE-UP, PMID 29766770) — the evidence base for the 4.5–9h / unknown-onset extended IVT window (COR 2a per AHA/ASA 2026).
- Corrected the `aha-asa-2026-4.6.3` title to "Late-window tenecteplase for LVO without thrombectomy access" so title matches its (unchanged) quote. The three late-window-TNK claims that cite it are now title/quote/claim consistent.
- Re-pointed the chip claim `ivt-window-4.5h` (standard tier → §4.6.1/§4.6.2; extended tier → EXTEND/WAKE-UP) and the live-page claim `late-window-selection-guideline-summary` (→ EXTEND/WAKE-UP) off §4.6.3.

**Re-review decision: approve** (clinical-reviewer, fresh context, model claude-opus-4-8). Block clears: the 4.5–9h tier is now faithfully sourced to EXTEND + WAKE-UP (titles, years, PMIDs, and verbatim quotes affirmed accurate against the stroke-guidelines skill and the NEJM publications). §4.6.3 is internally consistent and correctly confined to the late-window-TNK claims; the live-page card is accurate and citation-supported; both new citations are fresh (last_reviewed 2026-06-10, 36-month landmark-trial window). No never-drift category violated. Non-blocking follow-up (addressed): stale "§4.6.3" code comments in PatientContextPanel.tsx updated to reference EXTEND/WAKE-UP so the in-code rationale matches the corrected mapping.
