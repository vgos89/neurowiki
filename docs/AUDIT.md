...
4. Home pill active-state: cobalt fill -> spec slate-50 fill (HOME_SPEC §1.3.1); resolve the two unspecced anatomy inserts (SavedCasesTile, FavoritesPreview).
   ...
   **STATUS (2026-06-29):** PENDING V decision. The cobalt-to-slate-50 pill is a visible style change awaiting sign-off. The "unspecced" SavedCasesTile, FavoritesPreview, and FeaturedRail are intentional V features and will be KEPT; the spec should be updated to document them rather than removing them.
