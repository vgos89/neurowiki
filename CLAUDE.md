...
**Enforcement (hook tier, the only global tier per §2).** `npm run check:humanizer` runs in `.husky/pre-commit`. An em-dash in any scanned rendered string is an **ERROR that blocks the commit**; hard AI signal-phrases are ERROR too. The scanner **walks whole directories** rather than a fixed file list: `src/pages`, `src/components`, `src/data`, `src/seo`, `src/config` and `src/lib/cases`, scanning every `.ts`/`.tsx` under them, with comments, code and placeholders skipped by the extractors. A new authored-content surface inside those directories is therefore covered the moment it ships, with no registration step. Two deliberate exclusions: `src/lib/citations/` is a no-touch zone because `quoted_text` is verbatim external text, and test, type-declaration, dev and generated files are skipped. A new surface OUTSIDE those six directories is not covered and must be added to `SCAN_DIRS`. *(Corrected 2026-07-31: this section previously described a fixed `TARGETS` file list, which understated the wall. The list has been directory-walking since the scanner was extended; the contract had not caught up.)* "Humanizer was run" is no longer honor-based: the wall enforces the em-dash floor mechanically. The broader humanizer judgment (signal phrases, rule-of-three, voice) remains the authoring agent's responsibility, reinforced by the same hook's phrase checks.
