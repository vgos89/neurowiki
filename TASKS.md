...
- [2026-05-01] Build six question-detail pages (/trials/q/:questionId) — Class D-clinical, gated by clinical-reviewer. Question IDs: tpa-timing, lvo-evt, anticoagulation, hemicraniectomy, bp-control, dapt (defined in src/data/trial-questions.ts). Each page synthesizes evidence from multiple trials. Requires medical-scientist authoring + clinical-reviewer approval + citation trace per TRIALS_SPEC §L5.3. (parked during: W7.1 spec amendment)
