...
- **Status:** [ ] open. The 15 BLOCKER findings were remediated (3b885b4, live-verified), 62 of the remaining 160 were addressed in `trial-library-tail-remediation`, and a further 14 across six foundational EVT records (MR CLEAN, MR CLEAN-NO IV, SWIFT PRIME, REVASCAT, BEST, BEST-II, BEST-MSU, plus LASTE and TENSION from a systemic sweep) were corrected on 2026-07-31: see `docs/reviews/clinical-PR-trial-audit-tail-six-records.md`. In that batch 10 audit findings confirmed, **1 REFUTED** (the audit was wrong and the repo was right), 3 new defects found that the audit missed, 4 left unresolved rather than guessed. **98 findings were never examined** (verification shards failed mid-run) and remain open, plus the 4 confirmed false positives which need no action. Work trial-by-trial, all surfaces of one trial at a time; the wide sweep injected new defects in five of six rounds. Full evidence and per-finding contradicting sources are in the audit artifact.
  ...
  - **2026-08-05, GUIDELINE DOCUMENTS SUPPLIED BY V.** The 2022 AHA/ASA Spontaneous ICH guideline, the 2026 ACC/AHA Dyslipidemia guideline and the 2024 AHA/ASA PRIMARY Prevention guideline are now in `Articles/Guidelines/`. Extracted text lives in the session scratchpad; re-extract with `pdftotext -layout` if needed. Commit `839ac30`.
    ...
    - **Secondary prevention (7 claims): STILL BLOCKED.** The 2024 PRIMARY prevention guideline does NOT cover them. They are post-stroke questions (duration of cardiac monitoring for AF after cryptogenic stroke, bubble-study echo, follow-up HbA1c) and need the **2021 AHA/ASA Secondary Prevention guideline (Kleindorfer, Stroke 2021;52:e364-e467)**, or a newer edition if one exists. Primary and secondary prevention are different questions; do not settle one with the other.
  ...
  - **Still blocked on a source we do not hold:** the 2022 AHA/ASA Spontaneous ICH Guideline (Greenberg et al., Stroke 2022;53:e282-e361) is not in `Articles/Guidelines/`. The ICH blood-pressure strength correction was settled 2-surfaces-against-1 in the conservative direction (Class IIb / Level B-R, harm floor <130, matching the ICH guide page and the pearl's own `evidenceLevel`), not against the published text. Re-verify when the document is available.
...
### hamlet-citation-13.6-completion — Class E-clinical [from clinical review 2026-07-27]
...
### aha-2026-4.7.4-strength-unconfirmed — Class E-clinical [from clinical review 2026-07-27, re-review round 2]
...
### compass-directsafe-unsourced-values — Class C-clinical [from clinical review 2026-07-27]
...
- **Why it is not already closed:** the unit tests mock `document.execCommand` and `HTMLTextAreaElement.setSelectionRange` because jsdom implements neither. The suite proves the call ordering is gesture-safe; it cannot prove WebKit honours the contentEditable+readOnly+Range recipe, or that iOS does not revoke activation partway. The bug was reported from an iPhone, so static + Chromium evidence is not sufficient to call it closed.
- **Acceptance checks:** on a real iPhone, (a) Safari: Copy lands the full block in the clipboard and shows the success toast; (b) an in-app WKWebView (LinkedIn/Instagram/Facebook): Copy either lands via the legacy fallback OR shows "Copy failed. Use Send instead."; (c) Send's failure path shows "Send failed. Open this page in your browser." rather than nothing; (d) no keyboard flash, scroll jump, or focus loss when the offscreen textarea is inserted.
- **Clinical impact:** high — this is the EMR documentation export path.
- **Rollback plan:** n/a (verification only).

### clipboard-hardening-sweep — remaining raw-clipboard surfaces [P1]
- **Status:** `[x]` DONE — 53a92dd (phase 1) + 0810c8a (phase 2). See CONFIRMED CLEAN 2026-07-27.
- **User-visible goal:** No copy button anywhere in the app claims success when the copy failed, and none fails silently.
- **Scope:** Group 1 (raw `navigator.clipboard.writeText`, several with unconditional success indicators — same false-success bug NIHSS had): `NihssCalculatorEmbed.tsx:68`, `StatusEpilepticusPathway.tsx:205`, `ElanPathway.tsx:246`, `EvtPathway.tsx:970`, `MigrainePathway.tsx:513`, `EmBillingCalculator.tsx:1256`; plus `.then()`-without-`.catch()` at `ProtocolModal.tsx:117`, `PathwayCocktailSummary.tsx:44`, `StrokeBasicsWorkflowV2.tsx:833,875`. Group 2 (already on the safe helper but pass no `onFailure`, so failure is silent): the 12 calculators listed in §11 plus `ThrombolysisEligibilityModal`; also `AscvdRiskCalculator.tsx:375`, `ClinicHeadachePathwayV4.tsx:144`, `PostStrokeLipidManagement.tsx:140` (`?.writeText(...).catch(() => {})` — safe but swallows).
- **Note for the implementer:** §10.3 bans the em-dash in rendered strings and `check:humanizer` blocks the commit. Match the NIHSS wording pattern.
- **Clinical impact:** medium-high (EMR export paths). **Rollback plan:** single revert.

### toast-live-region-a11y — calculator toasts may not reach VoiceOver [P2]
- **Status:** `[x]` DONE — 2a62443 (group A) + 3d69f97 (group B). Scope turned out to be 10 surfaces, not one. See CONFIRMED CLEAN 2026-07-27.
- **Finding (mobile-first review, 2026-07-27):** `CalculatorToast.tsx:19-31` does `if (!message) return null`, so the `role="status" aria-live="polite"` node is portal-mounted only when a toast fires and unmounted when it clears. Screen readers reliably announce text mutations inside an already-present live region, not wholesale node insertion. Pre-existing and systemic (all 12 calculator pages), but newly load-bearing: the copy-failure toast is now the only channel telling a clinician their EMR copy did not work, so a VoiceOver user would get no feedback at all.
- **Fix:** render the container unconditionally, toggle only text/visibility. Route through `accessibility-specialist` + `accessibility-audit`.
- **Clinical impact:** low-medium. **Rollback plan:** single revert.

### pfo-trials-program — multi-phase PFO/stroke build-out [V-approved 2026-07-23: build full program, phased; approval gate before each Phase 2+]
- **Status:** Phase 1 shipped (6773dd0, live-verified); Phases 2–5 planned, `blocked:awaiting-V-approval` (one gate per phase).
- **User-visible goal:** Bring PFO/stroke trial coverage to a complete, current-to-2026 reference across the cryptogenic-closure question and the adjacent clinical questions V supplied source material for.
- **Source:** V-supplied comprehensive PFO/stroke overview (2026-07-22 session) — treat every trial datum as a hypothesis to verify (evidence-verifier) before authoring, per §19.0. The overview contained truncation artifacts (cut-off CIs/P-values), so no stat ships un-reverified against the primary source.
- **Phase 1 — Class C-clinical [x] done 6773dd0:** Completed the closure history on `pfo-closure-cryptogenic` — attached 4 precursor RCTs (CLOSURE-I 2012, PC 2013, RESPECT original 2013, DEFENSE-PFO 2018); `trialCount` 3 → 7; `meta` rewrite to the missed-primary-endpoint → positive arc. No new trials/citations. clinical-reviewer approve — docs/reviews/clinical-pfo-closure-history-phase1.md. Gates: tsc / build (173 routes, 0 failed) / claims / humanizer / card-meta / chains / routes green. Gate 6 live-verify PASS — /trials/q/pfo-closure-cryptogenic renders all 7 trials + the "Synthesises 7 trials" badge + updated eyebrow live.
- **Phase 2 follow-ups — Class C-clinical [ ] open [from clinical review 2026-07-27]:** (a) `trialData.ts` REDUCE `educationalContext` calls REDUCE "the strongest of the three 2017 trials", contradicting CLOSE (HR 0.03) and the synthesis prose: two rendered surfaces disagree, route to medical-scientist. PRE-EXISTING, outside the Phase 2 diff. (b) Attach the 24-month horizon to the REDUCE NNT ~28 figure in the synthesis (it is a 24-month KM-derived value). (c) Cite or soften "prolonged ECG monitoring must be completed before calling a stroke cryptogenic": no registered quoted_text among the 15 covers cardiac monitoring. (d) Consider adding the RESPECT long-term exploratory / group-sequential caveat (P=0.046 vs threshold 0.043). (e) Consider explicit `review_window_months` on the 2021 anchor guideline (§13.7 rapidly-evolving area). (f) ui-architect: 15 citation chips render full long titles when a citation has no `section` (cosmetic). (g) evidence-verifier: confirm the 2026 PASCAL category-level ABSOLUTE percentages against the JAMA Neurology full text (with published CIs) before they may be displayed; restore path documented in the `saver-pascal-2026` registry comment.
- **Phase 2 — Class E [x] done 51647a8, live-verified:** Gates: tsc / build (173 routes, 0 failed) / claims / humanizer / card-meta / chains / trial-coverage / routes all green. Gate 6 PASS on production with the synthesis card expanded (the card is collapsed-by-default per the 2026-05-23 layout decision, so the body paragraphs are client-rendered and absent from prerendered HTML: verify by expanding, not by static fetch). Confirmed live: both defects ABSENT (no "59 to 83" range, no "halted early for efficacy"); corrected AF sentence present ("83% of that new AF was detected within 45 days", "59% had resolved within 2 weeks of onset"); SCOPE 0.47%/1.09%, PASCAL categories, Danish cohort, 20-year cohort, REDUCE-MRI RR 0.44, device 90.6% + "technical surrogate", and "jointly with cardiology and the patient" all present. Brought the `pfo-closure-cryptogenic` synthesis to 2026. Registered 7 new citations (`kent-scope-2021`, `saver-pascal-2026`, `vidal-cales-pfo-20yr-2026`, `bonnesen-pfo-danish-2024`, `messe-reduce-mri-2021`, `zhang-biodegradable-pfo-2026`, `kent-pfo-review-2025`); claim `pfo-closure-cryptogenic-synthesis` extended 4 → 15 citation_ids; synthesis rewritten around causal-likelihood selection (SCOPE + PASCAL), durability, real-world residual risk, mechanism, and a bounded device note. Evidence packet: `docs/evidence-packets/2026-07-23-pfo-closure-refresh.md` (corrected 3 errors in the source material: PASCAL attribution, the unverified Danish person-years figure, the device trial year + surrogate endpoint). Clinical review: `docs/reviews/clinical-PR-pfo-closure-2026-refresh-phase2.md` (approve-with-conditions; all 4 conditions cleared pre-commit). **Two BLOCKER-grade defects found in PRE-EXISTING live prose and fixed here:** a fabricated cross-trial AF range ("59 to 83 percent detected within 45 days" fused REDUCE's 83%-detected-at-45d with its 59%-resolved-at-2wk) and an unsupported "DEFENSE-PFO halted early for efficacy" assertion.
- **Phase 2 (superseded planning entry) — Class E:** Bring the `pfo-closure-cryptogenic` synthesis to 2026 + modern pooled evidence — SCOPE 2021 individual-patient-data meta-analysis, SCOPE/PASCAL 2026 net-benefit (≈4/5 net benefit, 1/5 net harm), 20-year durability (JAMA Cardiol 2026), Danish real-world cohort 2024, REDUCE-MRI substudy 2021, biodegradable-device noninferiority RCT 2025. New citations; evidence-verifier → trial-statistician → medical-scientist → clinical-reviewer. Decide whether SCOPE/PASCAL renders as its own evidence card or stays in prose.
- **Phase 3 — Class E [x] merged 2026-07-31:** New question `pfo-closure-migraine` — MIST 2008, PRIMA 2016, PREMIUM 2017, pooled 2021; new synthesis; SEO route meta + JSON-LD schema + sitemap. Shipped after FOUR clinical-review rounds (artifact: `docs/reviews/clinical-PR-pfo-closure-migraine-phase3.md`), each of which blocked. Rounds 1-4 found: a delta band asserting benefit on a null co-primary; a bare "Negative" chip on a co-primary trial; three claims asserting guideline strengths their citations did not carry; an evidence packet with no §8 at all; a suppression note calling four randomized trials non-randomized; a half-applied authorship correction leaving broken sentences on six surfaces including a citation's quoted_text; a noninferiority verdict rendered as a p-value on five pages; and a harm assertion on PRISMS whose own interval crosses zero. All fixed and verified in the built HTML. Also corrected two live pre-existing defects outside PFO: ANNEXA-4 announcing "82 extra recoveries per 100 patients" against a single-arm study with no comparator, and OPTIMAL-BP calling a 15-point deficit "negligible" under its own stopped-for-safety callout.
- **Phase 4 — Class E [x] merged d65a21b 2026-07-31 (V chose a SEPARATE question + RoPE interlink):** Evidence packet COMPLETE at `docs/evidence-packets/2026-07-31-pfo-anticoagulation.md`. It found and we CORRECTED a wrong CI shipped live in 6 places (CLOSE anticoagulation arm HR 0.44, 95% CI 0.11-1.85 -> 0.11-1.48; commit 83d042f) plus a mis-scoped 'significance not analysed' on 4 surfaces. **VERDICT: no adequately powered trial has ever tested anticoagulation vs antiplatelet in a PFO-selected population.** Every dataset is a subgroup or an underpowered arm; all interaction tests null (0.18, 0.83, 0.28); the pooled signal DECAYED as trials accumulated (Kasner OR 0.48 in 2018 -> Diener OR 0.70 in 2021, same pool, same method); the bleeding excess is the reproducible finding (RR 1.69, 1.18-2.43). Two refinements to the expected null: a significant PFO-stratum signal DOES exist in 3 pooled analyses and must be reported not suppressed; and BMJ Rapid Recommendations 2018 DID make a weak recommendation FOR anticoagulation, so 'no guideline recommends it' is false as stated. **BLOCKED ON A DECISION (see below).** Remaining build: 3 new trial records (PICSS, NAVIGATE-ESUS, RE-SPECT ESUS) with full §21.1 five-surface wiring, 11 new citations, synthesis.
- **Phase 4 DECISION RESOLVED 2026-07-31, reconfirmed after a rubric-staleness mix-up — V chose a SEPARATE question (`pfo-antithrombotic-choice`) and asked for the RoPE interlink. V briefly answered the opposite while working down a stale rubric row that I had failed to refresh after the first answer; on being shown the conflict V confirmed 'keep it separate'. No reversal was performed and none is pending. Root cause was mine: the published rubric listed a decision as open after it had been made. Original framing:** the `anticoagulation` question currently holds three AF-TIMING trials (TIMING, OPTIMAS, ELAN). AF-timing and PFO-antithrombotic-choice are different bedside decisions sharing only the verb 'anticoagulate'. Options: (a) a distinct question, e.g. `pfo-antithrombotic-choice`, or (b) append to `anticoagulation` as a seven-trial mixed bag. Verifier and orchestrator both recommend (a). Product call for V.
- **Phase 4 follow-ups [ ] open:** (a) confirm CLOSE HR 0.44 CI against the NEJM primary text (currently on two secondary sources, one by the CLOSE PI); (b) resolve the PICSS endpoint conflict (Turc/Mas 4/42 vs 10/56 composite versus AAN 2/42 vs 8/56 stroke-only, both carrying the SAME HR and CI, which cannot both be right); (c) retrieve the Lancet Neurol 2018;17(12):e1 erratum to NAVIGATE-ESUS PFO, since every subgroup number comes from the corrected article; (d) retrieve the 47-patient Shariat trial citation, whose direction FAVOURS ASPIRIN and which is routinely omitted; (e) verify any AHA/ASA 2021 statement on antithrombotic CHOICE before publishing one (the existing repo quote is scoped to closure and must not be stretched).
- **Phase 4 (superseded planning entry) — Class E:** Feed the existing `anticoagulation` question with PFO-relevant trials — PICSS/WARSS substudy 2002, CLOSE anticoagulation arm, NAVIGATE-ESUS PFO subgroup 2018, RESPECT-ESUS PFO subgroup 2019, 2025 meta-analyses. Resolves the existing TODO in trial-questions.ts (anticoagulation question).
- **Phase 5 — Class E [~] PART A MERGED c9c6989 2026-07-31, PART B in progress:** **Part A, the guideline timeline, is DONE and live on all three PFO question pages.** Built entirely from six guideline-tier citations already registered and verified, so no new evidence work was needed. New reusable surface: `src/data/guidelineTimelines.ts` + `src/components/trials/GuidelineTimeline.tsx`, gated per question by `TrialQuestion.guidelineTimelineId`. The component reads `quoted_text` from CITATION_REGISTRY at render rather than restating it, so the surface stays downstream of the registry. Clinical review BLOCKED round 1 on five authored lines asserting more than their citations carry (artifact: `docs/reviews/clinical-PR-pfo-guideline-timeline.md`); all cleared and verified in the built HTML. Two of the five converted a not-located search result into an asserted fact, which is the same failure mode corrected on the migraine page earlier the same day. **Part B, the TEE / diagnosis packet, is with evidence-verifier** (`docs/evidence-packets/2026-07-31-pfo-diagnosis-imaging.md`), commissioned to test whether the 33.8%-vs-7.4%/12.6% ascertainment argument we currently publish is sound, overstated or understated. **Remaining for Phase 5:** the RoPE/PASCAL explainer surfaces. Note RoPE is now interlinked with all three PFO questions in both directions (9511c0c), so the explainer is the only piece left. Original framing:** Context surfaces — guideline timeline (AAN 2016/2020, AHA/ASA 2021, SCAI 2019/2022, European position 2019, Canadian 2017, EuroIntervention 2025, VA/DoD 2023), TEE diagnostic role, PASCAL/RoPE explainers. Some may not fit the trial/question card structure → may file `blocked:awaiting-scanner-support` per §13.3. RoPE calculator already exists.

### woven-trial-card-phase2 — Class D-clinical [from ICAD refresh Phase 1 clinical + architect review 2026-07-22]
- **Status:** [ ] open — Phase 2 of the ICAD refresh (Phase 1 shipped 5f35d1f + 4561456).
- **User-visible goal:** Add the WOVEN 2021 trial card (1-year single-arm follow-up of the WEAVE on-label Wingspan cohort, 8.5% stroke/death, 11/129) to the trials catalogue and the `icas-stenting` question rail, displayed honestly as a single-group safety result. WOVEN is currently cited in the icas-stenting synthesis prose but has no card, because the trial-page renderer would draw it as a false two-bar comparison against the SAMMPRIS medical arm.
- **Non-goals:** No change to CASSISS/BASIS/SAMMPRIS/WEAVE cards; no new clinical claims beyond the already-verified WOVEN data (evidence packet 2026-07-22-icad-refresh.md §3, PMID 32561658).
- **Files likely touched:** src/pages/trials/TrialPageNew.tsx (generalize the archetype-G render dispatch at ~line 4242 from a hardcoded `trialId === 'weave-trial'` to also fire for WOVEN, or a single-arm variant), src/components/trials/archetypes/BenchmarkThresholdChart.tsx (add a no-pre-specified-threshold mode: render the SAMMPRIS-medical 12.2% line as a labeled illustrative historical reference with NO "Benchmark met/failed" pass-fail pill), src/data/trialData.ts (re-add the woven-trial record; the Phase-1 authored record is in git history), trialCatalogMeta.ts + trialListData.ts (category) + trial-questions.ts (icas-stenting trialIds → 5, trialCount 5) + public/sitemap.xml + regenerate cardmeta, src/lib/citations/claims.ts (re-add woven-primary-result; alexander-woven-2021 citation already registered).
- **Acceptance checks:** WOVEN renders as a single-arm result (no two-bar efficacyResults comparison); the SAMMPRIS reference line is explicitly labeled historical/illustrative with no pass-fail pill; WEAVE's own archetype-G render is not regressed; ui-architect + trial-statistician + clinical-reviewer §17.2 sign-off; tsc/build/claims/humanizer/card-meta/routes green; Gate 6 live-verify on /trials/woven-trial.
- **Clinical impact:** low (WOVEN is confirmatory single-arm safety context; the clinical conclusion is unchanged whether or not the card ships).
- **Rollback plan:** git revert the Phase-2 commit(s); the render-path change is additive/guarded.
- **Source review:** docs/reviews/clinical-PR-icad-refresh.md (Required follow-ups: WOVEN Phase 2) + docs/reviews/arch-PR-icad-refresh.md; evidence packet 2026-07-22-icad-refresh.md §3.
