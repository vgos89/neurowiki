...
### basilar-mrs-gate-followup — Class E-clinical [ ] open, HIGHEST PRIORITY [from clinical review 2026-07-23]
- **Status:** [ ] open. Same clinical error class that justified blocking the guideline-mirror correction, on six surfaces the mirror fix did not cover.
- **Issue:** the basilar EVT Class I recommendation is stated WITHOUT its `baseline mRS 0 to 1` gate on: three Study Mode pearls in `src/data/strokeClinicalPearls.ts`; `src/data/guideContent.ts` ("Strong evidence for EVT up to 24 hours"); `src/pages/guide/Thrombectomy.tsx` ("Basilar: EVT up to 24 h", conspicuous because every sibling statement on that page carries COR and gates); and `src/seo/routeMeta.ts` ("EVT now recommended up to 24 hours"), which renders as a search-result snippet. Without the gate these extend a Class I recommendation to prestroke mRS >=2, which Figure 3 of the guideline assigns to "insufficient data to determine", and they contradict the EVT pathway calculator on the same site.
- **Also:** these three pearl claims (`baoche-posterior-evt`, `attention-posterior-evt`, `posterior-circulation-evt-quick-claim`) map to `aha-asa-2026-4.7.3`, whose refreshed quoted_text now carries the gate verbatim. §13.6 step 3 for that citation is recorded PARTIAL until these close.
- **Acceptance checks:** gate added on all six surfaces; §13.6 step 3 upgraded to complete in the registry comment; §17.2 clinical review; gates + Gate 6.
- **Clinical impact:** medium to high (recommendation applied to a population the guideline does not address).
...
### deltabandchart-display-defects — Class C [ ] open [found 2026-07-23]
...
### trialpage-duplicate-render-blocks — Class D [ ] open [found 2026-07-23]
...
### hook-coverage-gaps — Class C [ ] open [found 2026-07-23]
