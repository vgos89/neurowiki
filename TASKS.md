...
### IMAGING-READ-CT-HEAD — "Read the Scan" CT head teaching module (v1 template) — Class D / E-clinical
- **Status:** blocked:awaiting-clinical-review (implementation committed to branch claude/imaging-reading-checklists-ponn3g; merge to main gated on citation source-confirmation)
- **User-visible goal:** a new bedside-and-teaching tool at /guide/read-the-scan/ct-head that teaches reading a non-contrast CT head with one fixed search pattern ("Blood Can Be Very Bad": blood, cisterns, brain, ventricles, bone). Quick mode is a terse bedside checklist; Detailed mode expands each step into the anatomy to inspect, terminology, normal vs abnormal, pitfalls, four window settings (brain, stroke, subdural, bone), and license-clean teaching diagrams. Hub at /guide/read-the-scan lists CT head as available with CTA/MRI-brain/MRI-spine coming soon. This module is the reusable template later modalities inherit.
- **Non-goals:** no CTA/MRI/spine modules yet; no self-test/quiz; no DICOM viewer; real photographic CT examples deferred (environment could not download Wikimedia images) and ship as pending placeholders with a verified license manifest.
- **Files:** src/data/imaging/{types,ctHead}.ts; src/components/imaging/{ImagingReadPage,Figure}.tsx; src/pages/guide/{ReadTheScanHub,ReadCtHead}.tsx; src/config/routeManifest.ts; src/App.tsx; src/lib/citations/{registry,claims}.ts (+13 citations, +37 claims, reuse aspects-evt-eligibility-2026); src/data/medicalGlossary.ts (+21 radiology terms); public/imaging/schematics/*.svg (4 diagrams); docs/adrs/2026-07-09-imaging-read-the-scan.md; docs/reviews/{arch,clinical}-imaging-read-ct-head.md; docs/evidence-packets/imaging-read-ct-head.md; docs/imaging/ct-head-image-manifest.md
- **Acceptance checks:** tsc clean; build green; check:routes (50); check:claims pass; check:humanizer pass; architect §17.1 approve-with-conditions (C1-C4 applied before authoring); accessibility pass on all three new UI files (2 shared-file follow-ups routed to ui-architect); clinical §17.2 BLOCK (provenance, not medicine).
- **Clinical impact:** medium (new educational imaging surface; content clinically accurate and hedged per packet; every actionable statement citation-tagged).
- **Rollback plan:** purely additive. `git revert <commit>` removes src/data/imaging/, the two routes, new components/pages, +13 citations / +37 claims / +21 glossary terms, and the schematics. No shared abstraction modified, no data migration.
- **Progress 2026-07-13 (PubMed confirmation pass):** 4 primary-study citations CONFIRMED verbatim + re-dated (perron [PMID corrected 9795316->9795317], mainali, barber, truwit); 2 claims narrowed to source-supported wording (hyperdense-MCA, tonsillar herniation); 2 more provenance errors caught (statpearls-skull-fracture NBK470349 wrong; statpearls-ich-imaging 65-95 vs 50-70 HU discrepancy). Clinical §17.2 re-review: block narrowed but stands for merge.
- **Block-lifting condition (clinical §17.2, updated 2026-07-13) — 5 remaining, all provenance:** (1) [hard] source the correct StatPearls Skull Fracture chapter to replace wrong NBK470349 (underwrites 6/7 bone claims); (2) [hard] reconcile statpearls-ich-imaging HU (re-source 50-70 or align to source 65-95); (3) verbatim-confirm riascos-herniation (PMID 31589570) or narrow the 4 herniation claims; (4) verbatim-confirm statpearls-epidural-hematoma; (5) resolve the 5 radiopaedia-* citations (obtain egress + verbatim-confirm, or re-source to PubMed-indexed refs — Radiopaedia is not PubMed-indexed). All require full-text egress this environment lacks, or V-supplied sources. Then reset dates honestly and re-submit to clinical-reviewer. Merge to main stays gated until then.
- **Follow-ups:** (1) drop real Wikimedia CT photos per docs/imaging/ct-head-image-manifest.md (verify bylines/licenses at drop-in); (2) ui-architect: make shared Term.tsx keyboard-operable (button semantics + aria-expanded) and add aria-pressed to the ArticleLayout Quick/Detailed toggle + focus ring on its back button; (3) librarian: reconcile CLAUDE.md §10.3 + check-humanizer.mjs header comment (both describe a stale TARGETS list; scanner auto-walks src/data).
