...
- [2026-05-20] **Add "PFO closure for cryptogenic stroke" trial question** — Class C-clinical-editorial. The three 2017 PFO closure trials (CLOSE, RESPECT long-term, REDUCE) ship as standalone trial entries in this commit but do not fit any of the existing 20 entries in src/data/trial-questions.ts. A new question — likely "When does PFO closure reduce recurrent stroke?" or "Cryptogenic stroke with PFO: closure vs medical therapy?" — should group close-trial, respect-trial, reduce-trial (and predecessors CLOSURE-I 2012, original RESPECT 2013, PC 2013 if those are added later). Curated answer paragraph required per TRIALS_SPEC §L5.3; clinical-reviewer gate. trialIds + trialCount in trial-questions.ts; QuestionIconKey to be selected (likely 'waveform' or new 'cardiac' icon). (parked during: 2017 PFO closure cluster authoring)
- [2026-05-21] **PRIORITY: SPA pre-rendering / SSR for SEO** — RE-PRIORITIZED from 2026-05-13, then ATTEMPTED + FAILED autonomously overnight 2026-05-21. Architect review: docs/reviews/arch-PR-spa-prerendering-2026-05-21.md (approve-with-conditions). First attempt used existing `react-snap` 1.23.0 (already in package.json from a prior abandoned setup); local build produced JS SyntaxError "Unexpected token '.'" on 7 of 11 target routes because react-snap's bundled Puppeteer Chromium is too old to parse modern JS (optional chaining `?.`, nullish coalescing `??`, etc. — widely used in React 19 code). Only 4 hub routes were "crawled" and all retained the SHELL homepage title because the JS crashed before Seo.tsx could update document.title. Per architect's explicit hard prereq ("if puppeteer/Chromium fails locally, do not push"), the postbuild script was reverted (commit pending). The reactSnap config block in package.json was kept with corrected paths + defensive flags for future iteration. THREE PATHS FORWARD for V to pick from in morning: (1) **Replace bundled Chromium with modern puppeteer** — install `puppeteer` ^22 explicitly, point react-snap at it via env vars; ~half-session of work, may still fail on other react-snap rot since the lib is unmaintained since 2019. (2) **Write a custom prerender script** — small Node script using modern puppeteer + chrome-launcher, ~100 lines, reads routes from routeManifest, snapshots after hydration delay, writes per-route index.html. Class C. ~1 session. Best maintained option. (3) **Switch to vite-plugin-ssg** — true build-time SSG; requires SSR-safe components (66 window/localStorage references across 20 files would need audit + guards). Class D, biggest refactor, but most correct architecturally. Recommendation: try (2) first — modern puppeteer + custom script gives us control + no dependency on dead tooling. (parked during: 2026-05-13 SEO Phase 1 overnight; re-flagged 2026-05-21 via Codex GCS keyword research; first execution attempt failed 2026-05-21 overnight)
- [2026-05-21] **GCS calculator H1 visibility decision** — Class C. Codex GCS keyword audit found the GCS calculator's H1 ("Glasgow Coma Scale (GCS) Score Calculator") is present in DOM but `sr-only` (visually hidden, screen-reader-only). Google still reads it from the DOM, but visitors land on the page and see "Eye Opening (E)" as the first visible heading instead of the page-level H1. Hurts perceived authority + may reduce click-through if Google's SERP snippet preview leads with a title the visitor then doesn't see on landing. Decision options: (a) make H1 visible above the sticky calculator header — design change requires V sign-off; (b) keep H1 sr-only but ensure the first visible H2 carries the keyword (already does — "Eye Opening" implicitly clinical); (c) restructure header to include a visible "Glasgow Coma Scale (GCS)" eyebrow + the existing sticky score chip. Recommendation: option (c) — adds a single visible H1 line above the sticky header without changing the existing calculator UX. Estimated effort: ~1 session. Same pattern likely applies to other calculator pages (NIHSS, ICH, ABCD2, etc.) — extend the fix as a Class C-clinical batch if the pattern holds. (parked during: 2026-05-21 GCS keyword research)
