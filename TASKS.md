...
- [2026-05-13] **7 WCAG 2.1 AA high-priority a11y failures** — **SAFETY-CRITICAL BATCH DONE (verified 2026-06-03); REMAINDER NEEDS FRESH RE-AUDIT.** The three modal findings (H1 ThrombectomyPathwayModal, H2 ThrombolysisEligibilityModal, H7 FeedbackModal — focus traps + aria-modal + focus-visible rings) were resolved by the 2026-05-17 `useModalFocusTrap` consolidation pass ("B-1"/"B-3" stroke-code a11y fixes); confirmed in-code 2026-06-03 and marked RESOLVED in docs/L5-accessibility-audit.md. The remaining items (H3 pathway aria-live result regions — now partially present in Migraine/SE/ELAN; H4 star aria-label; H5 StrokeBasicsWorkflowV2 toggle/tab ARIA; H6 LKWTimePicker keyboard access) reference at least one file that has since been renamed/removed (GCAPathway.tsx no longer at the cited path), so the 2026-05-13 line numbers are stale. NEXT: commission a fresh accessibility-specialist re-audit of the pathway/time-picker surfaces before implementing H3–H6. (parked during: L5 a11y audit overnight)
