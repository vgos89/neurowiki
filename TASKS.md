...
- [2026-05-21] **SPA pre-rendering (Path B) — local-validated, awaiting puppeteer upgrade to ship to Vercel.** History: parked 2026-05-13, re-prioritized 2026-05-21 via Codex GCS audit, Path A (react-snap) attempted + failed overnight (bundled 2018-era Chromium can't parse React 19 JS), V chose Path B (custom script). Architect review: docs/reviews/arch-PR-spa-prerendering-2026-05-21.md. **Status: scripts/prerender.mjs shipped + validated locally on 11 Phase-1 routes** — all 11 produce correct per-route titles in static HTML (verified: GCS, NIHSS, ASPECTS, ICH, ABCD2, HAS-BLED + 4 hubs + home all now have route-specific `<title>` baked in). Uses dynamic puppeteer import + resolves system Chrome by platform (macOS /Applications/Google Chrome.app, Linux /usr/bin/google-chrome-stable, PUPPETEER_EXECUTABLE_PATH override). Works locally tonight via the old bundled puppeteer 1.20.0 (transitive via react-snap) + system Chrome — DevTools Protocol is backward-compatible. **NEXT STEPS FOR V (morning, ~10 min):** (1) `npm install -D puppeteer` (latest, ~24.x — bundles its own Chromium, works on Vercel build runners). (2) Verify locally: `npm run build && node scripts/prerender.mjs` should still succeed. (3) Add `"postbuild": "node scripts/prerender.mjs"` to package.json scripts. (4) Push. (5) Gate 6 live-verify per-route titles in production. Phase 2 (expand to all 43 static routes): generate include list programmatically from STATIC_ROUTE_DEFINITIONS. Phase 3 (dynamic trials + questions): curated allowlist by GA4 traffic. (parked during: overnight 2026-05-21; Path B local validation complete)
