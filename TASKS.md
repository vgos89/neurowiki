...
- **Status:** [~] in_progress — L4, P1. Result-presentation increment LANDED (c885da2): ranked phenotype accordion list (new `HeadacheResultList` + shared `CriteriaList`) replacing the stacked headline card + differential ribbon + multi-diagnosis banner; trials/calculator density; top match open; all result clinical strings relocated verbatim; a11y fixes (decorative criteria bar, sr-only live region, focus ring + touch-manipulation). Architect review approve-with-conditions (all met); fresh-context a11y + mobile sign-offs applied; Gate 6 client-side PASS (Migraine 4/4 open, Cluster 25% collapsed). REMAINING: (a) treatment link-out — replace inline `data-claim` management cards with links (next increment, Class D-clinical); (b) Stage Two result-copy — band words (Leading/Possible/Less likely), non-collapsible SNNOOP10 disclaimer, "considered and set aside" tray consuming `definitionallyExcluded`/`exclusionReason`, citation footer (Class E-clinical).
