...
- [2026-06-05] **Headache v4 rebuild follow-ups** — from W-HEADACHE-V4 (architect Q4 + `docs/reviews/clinical-headache-v4-postexec.md`). (1) Retire `src/components/pathways/MapperPanel.tsx`: now unused by the headache pathway (its only live consumer, `HeadacheResultList`, was deleted in the v4 rebuild); confirm no other live consumer, then remove (Class C, non-clinical). (2) Re-tag `clinic-headache-moh-gepant-safe` → `clinic-headache-migraine-acute`: the ID mislabels the migraine acute card (carried verbatim from the prior pathway); editorial registry rename, render-identical (Class C-clinical, clinical-reviewer sign-off). (3) Minor mobile z-order: the global feedback bubble slightly overlaps the "See result" control on the question-phase bottom bar at 375px — nudge the bar's right inset or the bubble (Class B). NOTE: the prior connectivity-audit headache findings **P-2/P-3/P-4** (old-page branch-chip no-op onClick) and **P-6** (old-page cascade Undo) are now OBSOLETE — the old `ClinicHeadachePathway.tsx` they referenced was deleted by this rebuild. (parked during: headache v4 build)
