...
- [2026-05-24] **Verify FDA Andexxa safety-communication URL resolves** — `fda-andexxa-safety-2024` citation registered with URL `https://www.fda.gov/safety/medical-product-safety-information/update-safety-andexxa-astrazeneca-fda-safety-communication`; the older slug `update-safety-andexxa` returns 404. The new slug is taken from current FDA search result; needs medical-scientist confirmation on a future fetch. Capture single verbatim quote on first successful access (current `quoted_text` joins two non-adjacent statements from secondary-source summaries). (parked during: Andexxa-withdrawal synthesis update, commit f35f255)
- [2026-05-24] **Find primary AstraZeneca BLA-withdrawal press release** — `astrazeneca-andexxa-withdrawal-2025` citation currently points to TCTMD secondary source for the Dec 22 2025 end-of-US-sales date. Add primary AstraZeneca press-release URL as a secondary citation field when located. (parked during: Andexxa-withdrawal synthesis update, commit f35f255)
