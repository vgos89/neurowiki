...
- [2026-05-13] **TrialPageNew per-archetype duplication** — **Phase 1 (sticky header bar) DONE — commit 4d4dfad (2026-06-03).** The architect (docs/reviews/arch-PR-trial-header-bar-extraction.md) split this into two phases. Phase 1 extracted the sticky header bar (back button + category badge), which was duplicated ~105× across archetype branches, into shared `src/components/trials/TrialHeaderBar.tsx`; a codemod (scripts/codemod-trial-header-bar.mjs) replaced 103 byte-identical inline blocks in 4 verified batches (net −823 lines), render-neutral across all 108 prerendered trial pages, clinical-reviewer approved (docs/reviews/clinical-PR-trial-header-bar-extraction.md). **Phase 2 (DEFERRED — own PR):** the `{trialMetadata.title}: {trialMetadata.subtitle}` H1 block (~17 branches, lines 476, 937, 1020, 1096, 1172, 1256, 1340, 1417, 1493, 1576, 1660, 1741, 1828, 1904, 1979, 2055, 2136, 2211, 2286…) → extract `TrialTitleHeading` with a tone/color prop (4 H1 color variants: #1746A2, isPositive ternary, #1e293b, isHarm ternary). Phase 2 also resolves the ADR-005 H1 question parked above. ~2 expanded multi-line header forms (e.g. EXTEND) were intentionally left for a follow-up as they don't match the compact codemod. (parked during: SEO Phase 1 overnight)
