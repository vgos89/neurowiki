...
- [2026-05-21] **PRIORITY: SPA pre-rendering / SSR for SEO** — RE-PRIORITIZED from 2026-05-13 parking lot. Codex GCS keyword research (486f792 source audit) confirmed concrete evidence of the parked issue: the live HTML title on /calculators/glasgow-coma-scale serves the generic homepage title ("Neuro Wiki: Neurology Resident & Attending Guide") instead of the GCS-specific route title from routeManifest. Per-route titles only appear AFTER JS hydrates client-side. Googlebot does execute JS but many initial-crawl and competitor-SEO tools see only the shell. This is the single biggest remaining SEO blocker across the site — affects all 43 routes' titles + descriptions + JSON-LD. Three approaches in increasing investment: (a) **vite-plugin-ssg** — adds build-time pre-rendering for the 43 static routes, lowest-effort, ~1 session of work + 1 round of testing; (b) **vite-plugin-ssr** — full SSR with hydration, more invasive; (c) **Vercel ISR / on-demand rendering** — platform-level solution requires Next.js or equivalent migration. Recommendation: ship (a) first. Estimated Class D — system-architect plan review needed before execution given blast radius. (parked during: 2026-05-13 SEO Phase 1 overnight; re-flagged 2026-05-21 via Codex GCS keyword research)
