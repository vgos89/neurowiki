...
- **Status:** in_progress — V approved the plan 2026-09-07; implemented + verified same day (typecheck clean, build 182/182 prerendered, 364/364 tests, live schema + crawl checks green, guard negative-tests pass); clinical-reviewer ratified the governance escalation boundary with 6 conditions, all applied. Scheduled task live at 07:50 (first unattended fire 2026-09-08 degraded gracefully: expired Google token + a foreign staged file, refused to commit, briefed both — designed behavior). Remaining: V's Google re-auth (`npm run seo:auth-login`), first data-fed supervised run, then three consecutive clean mornings; `seo:weekly` retirement at that mark per the ADR.
...
- **User-visible goal:** The late-window thrombolysis pathway stops telling clinicians that the 2026 AHA/ASA guideline restricts this treatment to two arteries. It does not: §4.6.3 rec 3 reads "AIS due to LVO" with no vessel and no circulation named. A patient with a basilar, vertebral or posterior-cerebral occlusion previously either got a red "Not Eligible", or (answering "yes, LVO confirmed", which is true) got an affirmative verdict whose justification described an internal carotid or middle cerebral occlusion they did not have, and `buildEmrText` exported that wrong-population justification verbatim into the chart. The LVO question now asks territory, and a non-anterior occlusion reaches a neutral "Outside Path C Scope" verdict that states the guideline position, the trial limit, the HOPE evidence, and the separate Class 1 basilar thrombectomy route under §4.7.3.
- **Non-goals:** no affirmative "Eligible" verdict for posterior circulation (clinical-reviewer declined on current evidence; no posterior-specific effect OR safety estimate exists); no NIHSS or prestroke-mRS gate added (§4.6.3 sets neither, and §4.6.1 rec 1 covers disabling deficits "regardless of NIHSS score"); no Path C window change from 9 h to 4.5 h (task C6); no HOPE claim mapping (task C4).
- **Files touched:** src/lib/citations/registry.ts (§4.6.3 corrected + retitled + pmid + all three recommendations verbatim; trace-iii-trial-2024 editorial text removed from quoted_text, Medium-confidence hedge closed against the open-access NEJM abstract); src/lib/citations/claims.ts (3 descriptions); src/data/strokeClinicalPearls.ts (3 rendered defects); src/pages/ExtendedIVTPathway.tsx (vessel misattribution cluster + new territory branch); docs/reviews/{arch,clinical}-PR-aha-4-6-3-recheck-vessel-attribution.md; docs/evidence-packets/2026-09-01-aha-4.6.3-recheck-and-eagle-comparison.md
- **Acceptance checks:** all passed — tsc clean; 288/288 unit tests; check:claims PASS (this citation's staleness had been blocking every commit in the repo); check:humanizer 0 errors; check:routes 48; check:trials 104; build 182/182 prerendered exit 0. Clinical review §17.2 approve-with-conditions (conditions 1-7 applied; C4/C6/posterior-affirmative excluded as separate tasks). Architect review §17.1 approve-with-conditions (A1, A2, A3, A4 applied before commit).
- **Clinical impact:** high. Corrects a false guideline attribution on a live treatment-decision surface, and removes a path by which a wrong-population thrombolysis justification could be written into a patient record.
- **Rollback plan:** `git revert <commit>` restores the prior quoted_text, the ICA/MCA-only Path C wording, and the boolean LVO question. The new `cLvoTerritory` state is additive and local to ExtendedIVTPathway; no persisted data carries it, so no migration is needed on revert. The citation `last_reviewed` reverts to 2026-05-19, which re-blocks commits on the stale-citation check until re-corrected.
...
- **User-visible goal:** On the Extended IVT (late-window thrombolysis) pathway: (1) the Path A DWI size question no longer reads as MCA-only, so a clinician with a PCA- or ACA-territory stroke sees the "< 1/3 of the MCA territory" figure framed as a volume yardstick that applies to any territory (WAKE-UP/THAWS threshold unchanged); (2) in the 9–24h window the large-vessel-occlusion (ICA/M1/M2) question is asked FIRST, before the penumbra question, surfacing the LVO gate up front, and the non-LVO verdict now states that a qualifying occlusion alone is not sufficient (penumbra still required); (3) every "Eligible" verdict now shows a qualitative symptomatic-intracranial-hemorrhage caution, and the 9–24h tenecteplase path additionally shows the TRACE-III figure (about 3% vs under 1%).
- **Non-goals:** no eligibility-threshold change; no NIHSS gate added (the pathway correctly has none); no per-path sICH numbers on Paths A/B (alteplase); TRACE-III number confined to Path C.
- **Files:** src/pages/ExtendedIVTPathway.tsx; src/lib/citations/claims.ts (2 new claims: extended-ivt-sich-caution, trace-iii-late-tnk-sich); src/lib/citations/registry.ts (trace-iii quoted_text numeric substantiation + aha-asa-2026-4.6.3 review_window_months:3); docs/reviews/{arch,clinical}-extivt-pathc-reorder-sich-caution.md; docs/evidence-packets/2026-07-19-extended-late-window-ivt-sich-caution.md
- **Acceptance checks:** all passed — tsc clean; build exit 0 (168/171 prerendered; 3 heavy hub routes hit the sandbox 30s prerender limit, tolerated 1.8%<5%, none touched by this diff); 261/261 unit tests pass; check:claims pass (2 new data-claim tags registered, all citations in-window); check:humanizer pass (0 errors, no em-dash); architect §17.1 approve-with-conditions (reorder change-list adopted; drawer placement single-source); clinical §17.2 approve-with-conditions (both conditions met: TRACE-III citation numeric substantiation + §4.6.3 3-month freshness override).
- **Clinical impact:** high (late-window IV-thrombolysis eligibility surface + new sICH safety caution; qualitative caution guideline-anchored to §4.6.1/§4.6.3, TRACE-III figure at Medium confidence and hedged).
- **Rollback plan:** `git revert <commit>` restores the MCA-only Path A wording, the penumbra-first Path C order, and removes the sICH caution + TRACE-III line. The two new claims and the registry edits are additive.
- **Follow-ups (non-blocking):** (a) consider `review_window_months:3` on trace-iii-trial-2024 for consistency; (b) re-review §4.6.3/TRACE-III when primary NEJM full text + HOPE (JAMA 2025, PMID 40773205) are retrievable, to elevate the sICH figure to High and confirm the definition label; (c) at next §4.6 refresh, note the Path A PCA/ACA extrapolation and generalize the Path C non-LVO "beyond 9 hours" wording for the unknown-onset/wake-up sub-route.
...
- [2026-09-07] SEO snippet surfaces are clinically load-bearing but unscanned: clinical-reviewer's SEO-DAILY-PORT ratification (docs/reviews/clinical-ratification-seo-daily-governance.md) verified that `src/config/routeManifest.ts`, `src/seo/routeMeta.ts`, and `src/seo/schema.ts` carry live clinical claims (COR grades, trial verdicts, eligibility thresholds in titles/descriptions/QUESTION_META answers) yet none is in `.claude/rules/clinical-surfaces.md` and none carries claim tags, so check-claims cannot see drift there. Needs a data-architect scanner-extension pass (§13.3 new-surface protocol). Pre-existing gap, independent of the port. (parked during: SEO-DAILY-PORT)
...
- [ ] L4/P1 **TN dose-conflict adjudication** — Class E, blocked:awaiting-clinical-review (pre-gate C16a). AAN 2008 gives CBZ 200-1200 / OXC 600-1800 mg/day; Nahas Table 10-2 gives 300-800 / 600-1200. No dose renders on the TN card until adjudicated; both ranges held in their own quoted_text. Consider the R1 idea (a visible "no dose shown" row) as part of the fix, with its own review pass.
- [ ] L4/P1 **acquire Bendtsen EAN 2019 TN guideline** (Eur J Neurol 2019;26(6):831-849, doi 10.1111/ene.13950) — V to supply PDF (pre-gate C16b). Until read: gronseth-aan-efns-tn-2008 re-examined annually; every grade-bearing row keeps its "(AAN 2008)" vintage label.
- [ ] L4/P1 **do-snnoop10-2019 quoted_text editorial removal** — Class E-clinical (pre-gate C16c; two consecutive reviews have flagged it). The record's quoted_text contains NeuroWiki editorial ("NeuroWiki applies the more sensitive over-50 threshold...") inside a field the engine treats as verbatim external text — same defect family as the removed airway directive.
- [ ] L3/P2 **teachPearl / pitfalls: wire or delete** — Class C (pre-gate C16d; medical-scientist finding F1). Both Phenotype fields are dormant (no component reads them), so reviewed clinical prose there reaches no clinician — including TN's teachPearl, which still carries the unsourced red-flag list. Either wire into PhenotypeMatch + render, or delete the fields and relocate content to live surfaces.
- [ ] L3/P2 **Continuum citation-metadata sweep** — Class C-clinical (packet §9.1.E + pre-gate C14). burish-2024-continuum-cluster (DOI ...1412 resolves to Karsan, pages wrong) and burch-2024-continuum-acute (DOI ...1411 resolves to Burish) both carry drifted DOIs; goadsby-2024-continuum-indomethacin carries ...1415, which now collides with the corrected nahas record and needs re-verification. Three of three checked Continuum DOIs resolved to the wrong article; sweep every Continuum record against Crossref.
- [ ] L4/P2 **GPN vagal-syncope referral steer** — Class E, optional (pre-gate ruling on S9-10): if a monitoring/referral recommendation for the asystole/syncope variant is wanted, it needs its own source, packet, and review pass; do not author unsourced.
- [ ] L4/P1 **MigrainePathway.tsx TN protocol block: full re-attribution** — Class E-clinical (clinical post-gate P1, 2026-09-09). The P1 SUBTRACTION shipped 2026-09-09 (Level A/B badges and export grade strings removed; "p.298" locator removed). Still open on that surface: (a) re-attribute the grades to gronseth-aan-efns-tn-2008 with the classic-TN scope, which is the same cross-source merge with the Nahas dose range as the TN dose-conflict task above (do both in one pass); (b) the IV fosphenytoin 15-20 mg PE/kg / lidocaine 1.5-2 mg/kg doses are NOT in Nahas p. 478 (which only reports the infusions as useful) and the row was removed from the sibling card under BC-1/BC-2 (refractory gate; AAN Level U) - re-examine identically; (c) the block is untagged (no data-claim) on a live route - register a claim when re-authored.
- [ ] L3/P2 **prerender hardening** — Class C. `npm run build` exited 0 on 2026-09-08 while `scripts/prerender.mjs` logged "Connection closed" for three trial routes and emitted no pages for them (no "[prerender] Done" summary either). A network blip during the headless render silently drops pages; the build must fail (non-zero) when any sitemap route yields no index.html, so Vercel cannot publish a partial site.
- [ ] L2/P2 **GPN editorial: none outstanding** — the post-gate's three Class B restorations (open "such as" vagal list; "in the nerve distribution"; pifp-A over-width comment) shipped in the batch commit 2026-09-09.

### Headache clinic — ICHD-3 facial-pain expansion (13.2.1 GPN + 13.12 PIFP) — Class E
- **Status:** ready_for_review (post-execution clinical gate) — V approved 2026-09-08 ("start"); evidence packet + architect plan review (approve-with-conditions) + clinical pre-gate (approve-with-conditions, C1-C17) all complete; implemented same day with every blocking condition applied.
- **User-visible goal:** a face-pain patient who is not trigeminal neuralgia can still land somewhere honest: glossopharyngeal neuralgia (13.2.1, with treatment + safety/workup cards) and persistent idiopathic facial pain (13.12, criteria-only) become findable patterns, and a throat-territory patient can no longer collect a confident wrong TN match.
- **Non-goals:** 13.2.2 painful glossopharyngeal neuropathy (sibling entity, out of scope like 13.1.2); GPN aetiology subtype resolver (deferred with TN's); PIFP treatment card (no held source; ships criteria + workup gates only); any airway/aspiration content (unsourced, removed from the registry in this batch); monitoring/referral steer for the vagal-syncope variant (no held source; history-taking instruction only).
- **Files:** registry.ts (ichd3-2018 append; nahas-2024 four-field correction + quoted_text rewrite; NEW gronseth-aan-efns-tn-2008), claims.ts (4 new, 2 amended), clinicHeadacheData.ts (2 phenotypes, 8 chips, vestigial probable-* ids removed), headacheQuestions.ts (loc-throat-ear option, b-glossopharyngeal + b-pifp branches), headacheBanding.ts (+test), HeadacheManagement.tsx (TN card re-attributed to AAN 2008 with CTN scope + STN Level U row; ON card evidence-base correction; GPN cards), HeadacheResultV4.tsx (typed claim-marker Record + literal-span pin), reachability tests (W1-W10 + guards).
- **Acceptance checks:** W1/W8 GPN full + TN absent on both location routes; W3 PIFP full despite dur-continuous; W9/W10 silent-drop behaviours tested with teach offered; config invariant (loc-trigeminal-distribution only from tn-distribution); CHAPTER_ORDER permutation; MANAGED both-directions; marker pin; 18-phenotype inventory; all repo gates green.
- **Clinical impact:** high (new diagnostic patterns + corrections to live TN/ON treatment cards).
- **Rollback plan:** single `git revert` of the batch commit is complete and sufficient (additive unions, no rename, no schema, no persisted state; the pathway holds all answers in component-local state). ONE retroactive effect to know: the new q-location answer changes an existing single-select core screen, so an angle-of-the-jaw or ear presentation that previously answered "In the face: cheek, jaw, or upper lip" is now steered to the throat/ear answer, changing which branches fire for that presentation (this is the intended fix).
...
- [ ] [P2] **Posterior-circulation affirmative verdict** — blocked:awaiting-source. Needs a posterior-specific effect estimate. Gating retrieval is the HOPE supplementary appendix or SAP. NOTE: Figure 3's "other arteries" row will NOT answer it — that bucket reconstructs to 123 patients of whom only 24% are posterior, and HOPE's own anatomic split was proximal vs distal, never by circulation.
- [ ] [P2] **Retrieve the published guideline** (Stroke 2026;57(8):e316-e436, PMID 41582814) to replace the FIRST PROOF now held in `Articles/Guidelines/`. Clears the §13.6 step-2 caveat across all eight `aha-asa-2026-*` citations, and fixes every "PDF page eNN" comment reference, which is first-proof pagination that will not resolve against the published article.
- [ ] [P2] **`aha-asa-2026-4.6.2` provenance re-check** — same 2026-05-19 authoring batch as the defect corrected here, same single-sentence paraphrase shape, non-house grade format, no `pmid`. Not stale (due 2026-11-19), not mapped to the affected claims, but same-batch risk.
- [ ] [P2] **Tag `ExtendedIVTPathway.tsx` computed verdict strings with the `claim()` helper.** The helper exists at `src/lib/citations/claim.ts` and is used nowhere in that file, which is why every defect found in this review was invisible to the pre-commit hook.
- [ ] [P3] **A5 tech debt:** the `value={x === null ? null : x ? 'yes' : 'no'}` / `onChange={(val) => setX(val === 'yes')}` boolean row adapter is copied ~10 times (candidate `PathwayYesNoRow`); the verdict object literal is now 14 copies whose optional-field discipline both `buildEmrText` and `onResultChange` silently depend on.
...
### trial-library-tail-remediation — Class E-clinical [x] done 39a3d5b, live-verified
- **Gate 6 PASS** on production: `/trials/decimal-trial` shows the protocol 30 h window with the observed 20.5 h mean and 7 to 43 h range, and no "30-35"; `/trials/compass-trial` shows neither the 68.9/76.3 pair nor the directional "closed the gap" implicature, and carries the not-yet-confirmed statement instead. No degenerate "N/A-N/A" interval on either page.
- 62 of the 160 non-BLOCKER audit findings re-verified against primary sources; 57 confirmed wrong, **4 confirmed audit FALSE POSITIVES** (repo already correct), 133 corrections applied across 12 files. Clinical review: `docs/reviews/clinical-PR-trial-library-tail-remediation.md` (approve-with-conditions; all conditions cleared pre-commit). Gates: tsc / build 173 routes 0 failed / claims / humanizer / card-meta / chains / coverage / routes green.
- **INCOMPLETE BY DESIGN:** 11 of 20 verification shards failed on API connection errors, so **98 findings were never examined**. See `trial-library-audit-remaining`.
- **Method lesson (governing):** six review rounds, five of which caught a NEWLY INTRODUCED defect. Bulk agent-driven correction of clinical prose has a measurable defect-injection rate; the mechanical gates passed on every one of them. The only two rounds in this program that introduced nothing were single-topic and fixed every surface of that topic at once. **Work the remainder trial-by-trial, not in a wide sweep.**

### basilar-mrs-gate-followup — Class E-clinical [x] done 2026-07-30
- **Closed.** The basilar EVT Class 1 / LOE A recommendation was stated on multiple rendered surfaces without its `baseline mRS 0 to 1` gate, which extended a Class I recommendation to prestroke mRS >=2, the population Figure 3 assigns to "insufficient data to determine", and contradicted the EVT pathway calculator on the same site.
- **What was actually wrong (three distinct defects, not one):** (a) surfaces omitting the mRS gate entirely; (b) surfaces attaching Class 1 to the TRIAL enrolment windows (12 h ATTENTION, 6-24 h BAOCHE) rather than the guideline's single 24 h window; (c) a Class IIa verb ("is reasonable") against a COR 1 label. All three closed across Study Mode pearls, `guideContent.ts`, `pages/guide/Thrombectomy.tsx`, `seo/routeMeta.ts`, `seo/schema.ts` QUESTION_META, the BAOCHE bedsidePearl and bottomLineSummary, `EvtPathway.tsx`, and the guideline mindmap.
- **Also closed:** the NIHSS 6-9 citation (`aha-asa-2026-4.7.3-nihss-6-9`) was wired to the three claims whose rendered text asserts COR 2b (previously that assertion had no supporting citation); the three claim descriptions were aligned; and the temporal ANCHOR ("from symptom onset", not from last known well) was restored wherever a trial LKW window preceded the recommendation. That anchor matters clinically: LKW is earlier than onset, so an LKW reading makes patients look out-of-window and UNDER-treats.
- **§13.6 step 3 for `aha-asa-2026-4.7.3` is COMPLETE as of 2026-07-30** for the four mapped dependents, which is the only scope the record can verify. Untagged surfaces carry no `claimId` and remain invisible to the enumeration and the hook: see `hook-coverage-gaps`.
- **Review:** `docs/reviews/clinical-PR-basilar-mrs-gate-followup.md`. Five review rounds; four caught a real defect, including two where a claimed fix had not actually been applied to the intended line.

### pathway-clinical-audit — Class E-clinical [ ] APPROVED BY V 2026-08-03, phase 1 starting
- **Status:** in_progress — Phase 1
- **User-visible goal:** every instruction a clinician can act on at the bedside is checked against the published guideline it claims to follow, and carries a source that stops it drifting again.
- **Why V approved it:** V raised the concern unprompted, correctly. Measured rather than assumed: **160 directive clinical statements across 31 pathway surfaces, 44 citation tags between them, and 24 of the 31 surfaces carry NO citation at all.** The EVT pathway alone has 39 directive statements and zero. Untagged does not mean wrong; it means nothing has checked it and nothing can.
- **The evidence this is not theoretical:** the CodeModeStep4 defect found 2026-07-31 (a 45%-sensitive test described as the one that "detects PFO", plus an unverified 40% figure, plus no claimId) was on a bedside pathway surface, not a trial page.
- **THE SEQUENCING RULE, and the reason for it:** pathway content was validated against `src/data/aha2026StrokeGuideline.ts`, which was itself found WRONG (§4.7.3 basilar, corrected 33a8980) and of which **114 of 132 recommendations remain unverified against any source.** Auditing pathways against a bent yardstick is precisely how the current defects survived. **Fix the reference first.**
- **Phases:**
  0. **BLOCKER FOUND 2026-08-03 while scoping, ahead of Phase 1: the yardstick cannot currently be lined up against the pathways at all.** `EvtPathway.tsx` makes **120 guideline-strength assertions** (Class I x28, Class IIa x15, Class IIb x12, Class III x10, COR 1 x10, COR 2a x8, plus section refs, §4.7.2 alone x15) and carries **zero** claim tags. But `aha2026StrokeGuideline.ts` records **no section identifiers whatsoever**: its 132 recommendations carry only `cor`, `loe` and `text`, and the only occurrences of the string 4.7.x in the entire file sit inside a correction comment. So there is no mechanical way to ask whether the pathway's "§4.7.2, Class I" corresponds to anything in our own reference, let alone to the published guideline. **Phase 1 must therefore also add section identifiers to the mirror**, or Phase 2 has nothing to verify against and degrades into re-reading the guideline by hand for each of 120 assertions. This is the same class of defect as the original one: a reference that looks authoritative and cannot actually be checked.
  1. **Yardstick.** Raise the verified share of the guideline mirror. Read the PUBLISHED guideline, not the in-repo build-plan markdown that superficially resembles it. Every disagreement between source and mirror is a candidate live clinical defect and is the highest-value output. (First slice in flight in the 2026-08-03 parallel run.)
  2. **Acute stroke, in consequence order:** EvtPathway (39 directives, 0 tags) -> CodeModeStep1/2/4 + StrokeBasicsWorkflowV2 -> ExtendedIVTPathway. Each directive verified against primary guideline text, never against our own files.
  3. **Remaining pathways:** MigrainePathway, StatusEpilepticus, IchManagement/HemorrhageProtocol/StrokeIchProtocolStep, SeizureWorkup, HeadacheWorkup, Meningitis, the rest.
  4. **Tag as we go.** Every audited directive gets a registered claim, so the coverage meter ratchets rather than resetting. This is what converts a one-time cleanup into a durable guarantee, and it is the only phase that prevents a repeat.
- **Acceptance checks:** every directive on an audited surface either carries a claimId resolving to a citation whose quoted_text supports it, or is removed; the guideline-mirror verified count rises and is reported per build; no audited surface contradicts another surface on the same value.
- **Clinical impact:** HIGH. This is the layer that converts information into action.
- **Rollback plan:** per-surface. Each surface is an independent commit with its own clinical review artifact, so any one can be reverted without touching the others.
- **Known risk, from this session's pattern:** expect a meaningful false-positive rate in whatever audit produces the findings. Two reviewer-reported defects on foundational trials did not reproduce this week. Every finding is a hypothesis until independently re-checked against current source.

### guideline-mirror-SOURCE-LOCATED — Class E-clinical [x] first pass done 2026-08-04
- **THE PRIMARY SOURCE EXISTS AND IS NOT IN THIS REPO.** `/Users/vaibhav/Documents/NeuroWiki/Articles/Guidelines/2026-Guideline-for-acute-ischemic-stroke.pdf` (7.6 MB, First Proof, 17 Mar 2026). Verified authentic: AHA/ASA masthead, Prabhakaran et al. writing-group roster, DOI 10.1161/STR.0000000000000513 matching what the evidence packets cite. PDF page N maps 1:1 to printed page eN. **Use this. Do not use either in-repo markdown.**
- **BOTH in-repo markdown files are build plans, not the guideline.** `docs/2026-AHA-Stroke-guideline.md` is titled "Mindmap - Improvement Plan"; a second file at the repo root is "Mindmap - Full Improvement Plan (v2)". Both contain the literal TypeScript export blocks that were pasted into the mirror, and both carry "Files touched: src/data/aha2026StrokeGuideline.ts". Validating the mirror against either compares a transcription with its own draft. This is why the mirror went unchecked for months.
- **First pass: 67 recommendations across 21 mirror groups compared against the PDF. 8 disagreements, ALL CORRECTED 2026-08-04:**
  - **D1 FABRICATED RECOMMENDATION.** Head positioning carried a COR 2b / C-EO head-of-bed-elevation recommendation that does not exist in §4.2, while the genuine COR 3 / B-R head-down recommendation was absent entirely. Same failure mode as the §4.7.3 basilar defect. (Caveat kept: §6.1-6.2 Brain Swelling was not read, so a head-elevation statement may exist elsewhere; it does not exist there, at that strength.)
  - **D3 HARM SOFTENED TO NO BENEFIT.** Urinary catheterization was COR 3: No Benefit / B-R; the source says COR 3: **Harm** / C-LD. The guideline says this causes harm and the mirror said it merely does not help.
  - **D7 A CLASS 1 DIRECTIVE DEMOTED TO CLASS 2a** by merging two recommendations into one entry: "do not delay IVT to obtain MRI for microbleeds" is COR 1 / B-NR and was riding inside a COR 2a entry. Now split out at its real strength.
  - **D6 SCOPE DROPPED.** Oxygenation conflated THREE recommendations: it took rec 5's COR/LOE but dropped its "who are ineligible for EVT" scoping, generalising a narrow no-benefit statement to all nonhypoxic patients, which contradicts the COR 2b normobaric-hyperoxia-before-EVT recommendation. It then grafted on an SpO2 target from a COR 1 recommendation and rendered >94% as >=94%.
  - **D2** anticoagulation timing: LOE B-R -> **A**; "carefully selected (eg, milder severity)" qualifier restored; "a DOAC" widened back to "oral anticoagulation".
  - **D4** prophylactic antibiotics: LOE B-R -> **A**; text replaced (previous text carried the corruption "are should not", itself evidence of retyping rather than transcription).
  - **D5** early mobilization: LOE A -> **B-R**. Note this is the OPPOSITE direction to D2/D4, which is why the pass concluded LOE values were reconstructed rather than transcribed.
  - **D8 REFUTED 2026-08-04 by reading PDF page e44 directly. The mirror was RIGHT.** The published table reads: "(a) awake with stroke symptoms within 9 hours from midpoint of sleep or (b) are 4.5-9 hours from last known well". Our copy says exactly that. All THREE extended-window recommendations in §4.6.3 verified correct (2a/B-R DWI-FLAIR mismatch; 2a/B-R midpoint-of-sleep; 2b/B-R LVO 4.5-24 h who cannot receive EVT). The verifier flagged this with an explicit "re-read at higher magnification before any edit" rather than asserting a defect on a thrombolysis time anchor, and that caution was correct. Original framing kept below for the record.
  - **D8 (original, now refuted) had been NOT APPLIED, deliberately.** The extended-window time anchor reads "midpoint of sleep" where the source table appears to say "last known well". The verifier flagged that the (a)/(b) clauses read near-redundantly on a rendered PDF page and explicitly asked for a re-read at higher magnification before any edit. Not confident enough to change a thrombolysis time anchor. **Open.**
- **21 mirror groups confirmed CORRECT**, including all 8 EVT recommendations in §4.7.2 and both §4.7.3 posterior-circulation recommendations. **The latter independently confirms this week's basilar fix was right: the guideline stratifies by severity, not by time window.**
- **Not verified in this pass, stated rather than reconstructed:** the two §5.5 depression recommendations (that table is on e77; only the e78 synopsis was read).
- **SECOND STRUCTURAL FINDING, 2026-08-04: the mirror PARAPHRASES rather than transcribes, and the paraphrase can shift the strength register.** §4.6.3 rec 2 is COR 2a and the source ends "IV thrombolysis may be reasonable to improve functional outcomes"; the mirror renders it "IV thrombolysis may be considered." In guideline language "may be reasonable" is 2a and "may be considered" is 2b, so the text reads a class weaker than the COR field beside it. The COR/LOE fields in that section are all correct, so this is invisible to a COR/LOE comparison and would be missed by any check that only compares strengths. Two entries in the same file also carry the corruption "are should not", and one opens "If the patient have". Taken together these say the mirror was RETYPED FROM MEMORY OR SUMMARISED, not copied. That is the root cause behind D1 through D7 and it means COR/LOE agreement alone is not sufficient evidence a row is faithful. **Any future verification pass must compare TEXT as well as COR and LOE.**
- **Structural blocker still open:** the mirror records no section identifiers, so a pathway assertion citing "§4.7.2, Class I" still cannot be resolved mechanically. See pathway-clinical-audit Phase 0.

### guideline-mirror-verification-backlog — Class E-clinical [ ] open [from data-architect plan 2026-07-30]
- **The finding that reframes this:** `docs/2026-AHA-Stroke-guideline.md` and the root copy LOOK like the primary source but are not. They are titled "Mindmap — Improvement Plan", list `aha2026StrokeGuideline.ts` under "Files touched", and contain the code blocks that GENERATED it. Diffing the mirror against them compares a transcription with its own draft. No guideline PDF is checked into the repo. The only independently-reviewed guideline text here is `quoted_text` on registered citations.
- **This is why a wrong recommendation survived for months.** The file the repo tells everyone to validate against had never itself been validated, except piecemeal via the one PDF-verified evidence packet covering §4.7.
- **Measured coverage (derived from the real module, not regex):** the mirror holds **132 recommendations across 49 sections**. Only **18** are verbatim-verified against a registered citation. **114 have no independently-verified source and are checked by nothing.** Even "covered" sections are partial.
- **Now enforced:** `npm run check:guideline-mirror` (pre-commit) asserts COR/LOE agreement for every verified recommendation and holds a coverage floor via `scripts/guideline-mirror.lock.json`, so a verified entry cannot silently drift away from its source. It reports the unverified count on every run so the gap stays visible rather than being mistaken for validation.
- **The work:** raise coverage by reading the published guideline and recording a verified `quoted_text` per section, highest-clinical-stakes first (IVT decision-making and agent choice, EVT adults/large-core, antiplatelet, anticoagulation, BP general). Each new quote makes its section machine-checkable. This is medical-scientist + evidence-verifier work, not scanner work, and needs a real source: the published guideline or a page-verified packet, NOT the build-plan markdown.
- **Acceptance checks:** each newly-verified section raises the lock count; §17.2 review per batch; no `quoted_text` sourced from the build-plan markdown.
- **Clinical impact:** high. These are recommendation strengths on a bedside tool, and 114 of them currently rest on an unverified transcription.

### basilar-followups-remaining — Class C-clinical [ ] open [from clinical review 2026-07-30]
- (a) Align the BAOCHE enrolment-window anchor across the remaining trial-page mirrors (`trialData.ts` 8167/8216/8220/8221, `TrialPageNew.tsx:1331`, generated card meta) to the combined "from symptom onset or last seen well" phrasing now used in `strokeClinicalPearls.ts`. These are narrower-but-true, not contradictions.
- (b) The EVT pathway offers only 0-6h / 6-24h under a "Time from Last Known Well" label with no onset input, while the basilar recommendation is onset-anchored. The basilar branch is time-agnostic (reads only mRS, pc-ASPECTS, NIHSS) and LKW <= onset makes the gate conservative, so the verdict cannot change; copy mismatch only.
- (c) Tag the ATTENTION bedsidePearl with `bedsidePearlClaimId: 'attention-posterior-evt'` and declare BEDSIDE_PEARL_SURFACE on that claim. Same invisibility gap just closed for BAOCHE.
- (d) **Stale worktree** `.claude/worktrees/agent-ab9d815fae22ee79d/` still carries pre-fix text and violates §5 rule 8 (no worktrees). Deletion was declined as a destructive operation and needs an explicit decision. Round 4's wrong-branch failure is the precedent for why shadow checkouts are dangerous.

### aha2026-guideline-mirror-basilar — Class E-clinical [x] done 33a8980, live-verified
- **Gate 6 PASS** on production. `/trials/q/basilar-evt` renders BOTH strata with correct badges: COR 1 / LOE A (NIHSS ≥10) and COR 2B / LOE B-R (NIHSS 6 to 9), each carrying the verbatim recommendation with the baseline mRS 0 to 1 gate and PC-ASPECTS ≥6. The card previously rendered NO strength badge; the dangling "(Class I, Level A)" is gone. `/pathways/evt` FAQ no longer says "can be considered", no longer lists basilar in the anterior mRS 0-2 group, and now states the separate 0-24 h basilar rule with its gates.
- **Status:** [x] done. Corrected across four surfaces to COR 1 / LOE A (NIHSS >=10) and COR 2b / LOE B-R (NIHSS 6-9), single 24-hour window, with the baseline mRS 0-1 and PC-ASPECTS >=6 gates. The §4.7.3 citation quoted_text was replaced with PDF-verbatim text and the NIHSS 6-9 stratum registered as its own citation, which also fixed the public guideline card rendering NO strength badge. Clinical review: `docs/reviews/clinical-PR-basilar-evt-4-7-3-guideline-mirror.md` (approve-with-conditions; C1-C3 cleared pre-commit, C4 is Gate 6). Original status text follows for the record: `src/data/aha2026StrokeGuideline.ts` rendered §4.7.3 basilar EVT as COR 1/B-R within 6 h plus COR 2a/B-R for 6-24 h. That contradicts the PDF-verified packet `docs/evidence-packets/2026-05-15-evt-pathway-aha-2026-PDF-VERIFIED.md` and `registry.ts` (`aha-asa-2026-4.7.3`), both of which record COR 1 / LOE A for NIHSS >=10 within 24 h, and COR 2b / LOE B-R for NIHSS 6-9. The file also omits PC-ASPECTS entirely.
- **Why it matters beyond itself:** the file's own header calls it a validation reference that other content "MUST be cross-checked against", so auditors keep deriving FALSE POSITIVES from it. One such false positive nearly caused a recommendation-strength DOWNGRADE on a Study Mode pearl this session.
- **Acceptance checks:** replace the two entries with the verbatim §4.7.3 recommendations at the correct COR/LOE; confirm no downstream surface was downgraded on the strength of the wrong mirror; §17.2 clinical review.
- **Clinical impact:** high (recommendation strength on a thrombectomy indication).

### deltabandchart-display-defects — Class C [ ] open [found 2026-07-27]
- (a) The component labels any absolute difference under 2 points "Negligible absolute difference" with no direction awareness, so ENRICH (9.3% vs 18.0% 30-day mortality) and DEFENSE-PFO (0% vs 12.9%) both render "negligible" beneath large benefits. Needs a `lowerIsBetter` prop. (b) The band aria-label hardcodes "extra recoveries per 100 patients", so a screen reader announces harm as recoveries on PATCH. (c) The "Risk ratio" prefix is hardcoded, producing `Risk ratio aOR 2.05`, `Risk ratio ARR 52.8 pp`, `Risk ratio RD +7.7 pp`; an optional `effectLabel` prop now exists and just needs applying at those call sites.

### trialpage-duplicate-render-blocks — Class D [ ] open [found 2026-07-27]
- Eight trials have duplicate per-trial render blocks in `TrialPageNew.tsx`; the second copy of each is unreachable after the first early-returns (THEIA, IST, CAST, PROFESS, ANNEXA-I, ANNEXA-4, Sarode, PATCH). For IST and CAST the **live** block is the degraded one (placeholder intervals) while the dead duplicate holds the real CIs, so correct data exists in the file and is not rendered. An editor fixing a chart has a coin-flip chance of editing the copy that never renders. Route to system-architect.

### hook-coverage-gaps — Class C [ ] open [found 2026-07-27]
- (a) `scripts/check-humanizer.mjs` skips any extracted string under 10 characters, so a bare em-dash placeholder in a rendered string passes the em-dash wall. Check `countEmDashes` before the length short-circuit. (b) `scripts/check-claims.ts` matches only a quoted `claimId:` field, so the `/* claimId: … */` comment annotations on trial records are invisible to it; those IDs are absent from `CLAIM_REGISTRY` and COMPASS has no `registry.ts` record at all. Trial-record clinical prose is a §13.3 claim surface the hook does not actually cover.

### trial-display-schema-gaps — Class D [ ] open [from trial-statistician 2026-07-31]
- **Status:** planned
- **User-visible goal:** trial pages cannot silently misreport a result because the data model has no way to express what the trial actually did.
- **Non-goals:** re-authoring any trial's clinical content; these are structural.
- **Files likely touched:** `src/data/trialData.ts` (type defs), `src/pages/trials/TrialPageNew.tsx`, `src/components/trials/archetypes/`, `src/utils/trialNarrative.ts`, `scripts/check-claims.ts`
- **Acceptance checks:**
  1. `continuous-superiority` added to the `primaryDesign` union and a `continuous-delta` archetype built for continuous mean-difference primaries. PRIMA is the reference case. The archetype must show the estimand in words, per-arm values with the unit inside the value, **per-arm dispersion or an explicit "not recoverable"**, the between-arm difference as published or an explicit statement that none was, the p-value with the test named, and an explicit "no confidence interval published" marker. Prohibited: dot grid, percent sign, shared 0-100 axis, NNT, delta band.
  2. `coPrimary[]` array plus a `'co-primary-split'` sentinel on `primaryResult`, required whenever `coPrimary` is populated. PREMIUM is the reference case: efficacy not met, safety met. The sentinel is the enforcement — every existing consumer reading `primaryResult` alone receives a value it cannot map, so a half-truth becomes a loud unknown instead of a silent "not met". Retires the hand-written `resultBadgeOverride` at PREMIUM's `BottomLineDrawer`.
  3. `unit?: 'percent' | 'days-per-month' | 'points'` on `EfficacyResults`, read by `buildTrialSummaryItems` (`TrialPageNew.tsx:105`) and `buildHouseConclusion` (`trialNarrative.ts:23/27/31`); `DeltaBandChart` returns null with a dev-console error when `unit !== 'percent'`. **Latent today:** PRIMA stores DAYS in a field typed `percentage`, and the summary builder would render "2.9% with Amplatzer PFO Closure". It does not render only because PRIMA returns early from a dedicated block; one route refactor away from shipping.
...
### navigate-esus-within-trial-ascertainment — Class E-clinical [ ] blocked:awaiting-erratum
- **Status:** blocked. The replacement figures come from a corrected article whose correction has not been read.
- **Context:** the PFO-diagnosis packet found a materially STRONGER ascertainment comparator than the one we publish. We currently compare NAVIGATE-ESUS's 7.4% and RE-SPECT ESUS's 12.6% against PICSS's 33.8%, a cross-trial, cross-decade comparison. NAVIGATE-ESUS reports a **within-trial** contrast in the same patients: PFO detected in **313 (4.6%) by TTE and 379 (27.4%) by TOE**, with the authors stating they are "likely to have underestimated the prevalence of PFO". That is strictly stronger and the trial says it about itself. It also shows our current comparator mildly OVERSTATES the counterfactual: 27.4%, not 33.8%, is what that cohort showed when properly imaged.
- **Why blocked:** those figures come from an article carrying a formal erratum (*Lancet Neurol* 2018;17(12):e1, PMID 30366869) whose content has not been obtained. Every NAVIGATE-ESUS figure is provisional against it.
- **Also carried here, needs its own verification pass before shipping:** under-ascertainment does not merely shrink the ESUS PFO subgroups, it SELECTS them. Because TTE preferentially detects large shunts, those subgroups are probably enriched for exactly the anatomy CLOSE and DEFENSE-PFO chose. That changes the caveat from "the subgroup is too small" to "the subgroup is the wrong shape", which is the more damaging claim. The packet flags this as the verifier's inference, not a published finding.
- **Clinical impact:** low as shipped (the current argument is sound, just weaker than it could be).

### pfo-high-risk-not-one-entity — Class C-clinical [ ] open [from diagnosis packet 2026-07-31]
- **Status:** planned
- **Context:** CLOSE and DEFENSE-PFO both enrolled "high-risk PFO" using **non-overlapping definitions**. CLOSE used a flow threshold (>30 microbubbles in 3 cycles) or a 10 mm septal excursion; DEFENSE-PFO used three anatomic features and **no flow threshold at all**. A patient with a 2 mm PFO separation and 10 microbubbles is high-risk in DEFENSE-PFO and ineligible for CLOSE. Any surface that pools them under one "high-risk anatomy" label elides a real difference. Also: TCD grading scales run to >300 microbubbles while TEE scales top out at >25, so a TCD grade and a TEE grade must never be cross-walked. And atrial septal aneurysm is defined at >=10 mm by the trials versus Hanley's original >=15 mm, so ASA prevalence figures are threshold-dependent and not comparable across sources without the threshold stated.
- **Clinical impact:** low, but it is a real conceptual error a reader can carry into practice.

### respect-shunt-grading-unverified — Class C-clinical [ ] open [from diagnosis packet 2026-07-31]
- **Status:** blocked:awaiting-source
- **Context:** RESPECT's shunt-size grading could not be confirmed from NEJM (unretrievable), the AAN advisory, or NCT00465270. Secondary sources report 1-9 / 10-20 / >20; unconfirmed and **must not be published**. The record currently says only that RESPECT did not require a minimum shunt size, which is correct and safely conservative. Do not add grades without a primary read.
...
- **Context:** `safetyData` (a string) has no render path anywhere in the app; only `safetyProfile` does (`TrialPageNew.tsx:755`, 572, 10591). Records populating the former get `renderSafetySection` returning null. Found on the three PFO-antithrombotic records, but the quirk is repo-wide and pre-existing. No clinical content was lost on those three, because the NAVIGATE bleeding HR 2.05 (0.51 to 8.18) reaches the page through the footnote, pearls, limitations and bedside pearl, always with its interval in the same sentence. Audit which records populate which field, then either render `safetyData` or migrate it.
...
### huang-pooling-pairing-rule — Class E-clinical [ ] open [from clinical review 2026-07-31]
- **Status:** blocked:awaiting-evidence-verifier
...
- **Context:** the page now renders "Not a randomized comparison, so no difference band is drawn" roughly ten lines above "NNT ~13 to gain one additional excellent recovery (mRS 0-1)". The NNT *is* the absolute risk difference the note declines to draw, inverted. One of the two is wrong. Deciding which is a clinical question: whether alternating-week cluster allocation supports an absolute-risk-difference display at all. A committed 2026-05-20 review defended the NNT on the grounds that the primary is a dichotomized utility-weighted mRS; the trial-statistics skill bans NNT on non-randomized comparisons. This is the §3 escalation that was already recorded as unresolved for BEST-MSU and now has a visible symptom.
...
- **Context:** DECIMAL (band 53), DESTINY (41) and HAMLET (37) chart **6-month survival**, a secondary endpoint that was significant at p=0.001, while their `primaryResult` is `not-met` for the mRS primary. The band matches the charted endpoint and is defensible on that basis, which is why the 2026-07-31 band fix deliberately left them alone. But a delta band is arithmetically an absolute risk difference and therefore an NNT, and the trial-statistics rule bans an NNT derived from a secondary endpoint on a failed-primary trial. Needs an explicit decision rather than an inherited default.
- **Also in scope:** `trialNarrative.ts:31` prints an ARD with no interval for any trial routed to the generic fallback. The band `div` also hard-codes its `aria-label` as "N extra recoveries per 100 patients" regardless of what is charted, which is wrong for the SURVIVAL endpoints on these three trials: the defining caveat of the decompressive-craniectomy trials is that survival is gained with mRS 4-5 disability, so "53 extra recoveries" mischaracterises the result. Latent rather than live, because the band sits inside a `role="img"` element and the label is therefore not exposed to assistive technology at all. Fix the label to take the endpoint, and correct the JSDoc so the next reader does not over-trust the a11y path.
- **Also:** `check-claims.ts` Check 4 reads only `calculations.nnt`, so a literal NNT written into JSX is invisible to it (`TrialPageNew.tsx:2048` prints "Approx NNT ~12"). Same false-coverage class as the unscanned `/* claimId: … */` comments.
- **Also (doc):** CLAUDE.md §10.3 describes the humanizer scanner's TARGETS as a fixed file list; `check-humanizer.mjs` actually walks whole directories. The wall is wider than the contract documents. A CLAUDE.md edit is deliberate per §23, so this needs V's nod rather than a drive-by fix.
- **Clinical impact:** low.
- **Rollback plan:** n/a, display-only.

### chart-effect-label-systemic — Class C [x] done 83d042f [from clinical review 2026-07-31]
- **Systemic display defect.** Only 5 of roughly 60 `DeltaBandChart` call sites pass an explicit `effectLabel`; the rest inherit the `'Risk ratio'` default while embedding their own prefix in the value. Rendered result includes "Risk ratio HR 0.79", "Risk ratio RD +2.1 pp", "Risk ratio ARR 52.8 pp", "Risk ratio cOR 2.77", "Risk ratio 0/60 vs 6/60". A hazard ratio or a risk difference displayed under the words "Risk ratio" is a display error, and where the value is an odds ratio it OVERSTATES the effect a clinician reads off the page (REVASCAT: OR 2.1 vs a true risk ratio of about 1.55).
- **Method:** enumerate every call site and read what statistic it actually passes, as was done for the distribution sweep. Do not pattern-match: the correct label differs per trial. Consider making `effectLabel` required in the props type so the default cannot silently apply.
- **Clinical impact:** medium. No number is wrong; the label naming the number is.

### tension-sich-contradiction — Class C-clinical [ ] open [from clinical review 2026-07-31]
- `TrialPageNew.tsx` chart footer renders "sICH: 5% both arms" while `trialData.ts` (teaching card, one section below on the same page) says 6% with thrombectomy versus 5% with medical treatment. Pre-existing. Needs a full-text check of Bendszus Lancet 2023, then both surfaces aligned.

### chart-stats-have-no-source-record — Class C-clinical [ ] open [from clinical review 2026-07-31]
- The corrected chart estimates for MR CLEAN (adjusted OR 2.16, 95% CI 1.39-3.38), REVASCAT (2.1, 1.1-4.0) and SWIFT PRIME (RR 1.70, 1.23-2.33) exist only as JSX literals in the render file. They match the published values but have no record field, no evidence packet and no citation, so nothing can verify them mechanically. Give each a home in the data layer.

### trial-claim-comments-not-scanned — Class C [ ] open [from clinical review 2026-07-31, second sighting]
- `scripts/check-claims.ts` matches a QUOTED `claimId:` field. The `/* claimId: … | source: … */` COMMENT form used throughout `trialData.ts` (60+ instances) matches nothing, so every trial statistic in that file sits outside §13.5 metadata enforcement while appearing to a human reader to be tagged. This is the false-sense-of-coverage failure §13.3 exists to prevent. Route to data-architect. Previously raised in the tail-remediation review; raised again independently here.

### destiny-ii-stopping-reason-unsourced — Class C-clinical [ ] blocked:awaiting-source [2026-08-04]
- **Status:** blocked. The causal claim was REMOVED from three surfaces rather than replaced, because neither candidate is sourced.
- **Context:** the record asserted on five surfaces that DESTINY II "stopped early for enrollment difficulty". The audit proposed replacing that with "stopped early for efficacy at a prespecified sequential analysis". **Neither is supported by anything in this repo:** the registered citation `juttler-destiny-ii-2014` quoted_text describes the results and is silent on why the trial stopped, and no evidence packet covers it. Swapping one unsourced causal claim for another is the exact failure this session has been correcting, so the surfaces now state that it stopped early after 112 patients and say explicitly that the reason is not established here.
- **Also unverified and now not asserted:** the "planned 188" denominator, which appeared alongside the reason and has no registered source either.
- **To close:** read Jüttler et al., NEJM 2014, and record the stopping rule and the planned enrolment in the citation quoted_text.

### audit-findings-needing-a-source — Class E-clinical [ ] blocked:awaiting-source [from parallel run 2026-08-03]
- **Status:** blocked. Three HIGH-risk findings reproduce in current source but cannot be resolved from anything in this repo. Deliberately NOT applied. Each changes a clinical conclusion, and this session has twice seen agent-reported defects that did not reproduce, so none is being flipped on an agent's word.
- **1. RESCUE BT symptomatic ICH significance.** Five rendered surfaces assert sICH was "significantly higher with tirofiban (9.7% vs 6.4%, P = 0.04)". A sixth surface in the SAME record says "numerically higher". The record therefore contradicts itself about whether a drug-safety signal is real. The verifier proposed P=0.07 with 95% CI -0.2 to 6.8, i.e. NOT significant. **No RESCUE BT citation exists in the registry** (the only rescue-* entry is `yoshimura-rescue-japan-limit-2022`, a different trial), so the repo cannot adjudicate. Needs the publication. Note the asymmetry: asserting a significant harm signal that is not real overstates risk, but flipping it to non-significant if the P really is 0.04 understates a real one. Do not guess in either direction.
- **2. ICH blood-pressure target strength.** `strokeClinicalPearls.ts` asserts "Target SBP <140 within 1 hour (Class I, Level A)" while the same pearl's own `evidenceLevel` field says 'B'. The verifier reports the 2022 AHA/ASA recommendation is Class 2b, Level B-R for lowering toward 140 (range 130-150), with SBP <130 potentially harmful. If correct, the page overstates a bedside BP target from Class 2b to Class 1 and from Level B-R to Level A. **No registered ICH-BP citation was found** to adjudicate; the nearest registered quote covers AIS/IVT BP, a different recommendation. Needs the 2022 ICH guideline §5.1 text.
- **3. THALES guideline scope.** The page says AHA/ASA 2026 rates ticagrelor DAPT "COR 3: No Benefit for the general population". The in-repo mirror scopes that COR 3 to ticagrelor INSTEAD OF aspirin in minor stroke (NIHSS <=3) or high-risk TIA; THALES tested ticagrelor PLUS aspirin in a broader population (NIHSS <=5). If the mirror is right, the page applies a guideline verdict to a regimen it does not cover. Blocked behind the pathway audit Phase 1, since the mirror itself is only 18/132 verified and is the thing in question.

### trial-library-audit-remaining — Class C/E-clinical [from docs/reviews/audit-trial-library-semantic-2026-07-23.md]
- **Status:** [ ] open. The 15 BLOCKER findings were remediated (3b885b4, live-verified), 62 of the remaining 160 were addressed in `trial-library-tail-remediation`, and a further 14 across six foundational EVT records (MR CLEAN, MR CLEAN-NO IV, SWIFT PRIME, REVASCAT, BEST, BEST-II, BEST-MSU, plus LASTE and TENSION from a systemic sweep) were corrected on 2026-07-31: see `docs/reviews/clinical-PR-trial-audit-tail-six-records.md`. In that batch 10 audit findings confirmed, **1 REFUTED** (the audit was wrong and the repo was right), 3 new defects found that the audit missed, 4 left unresolved rather than guessed. **98 findings were never examined** (verification shards failed mid-run) and remain open, plus the 4 confirmed false positives which need no action. Work trial-by-trial, all surfaces of one trial at a time; the wide sweep injected new defects in five of six rounds. Full evidence and per-finding contradicting sources are in the audit artifact.
  - **2026-08-04 progress.** Two further trial-by-trial batches shipped. `238f040` corrected 8 records (CREST-2 annual-vs-4-year rate comparison, EXTEND-IA unsourced risk ratio and CI removed, IMS-III eligibility, NOR-TEST self-refuting exclusion, ATTENTION mortality magnitude, NOR-TEST 2 Part A safety multiples, TRACE-III endpoint naming, THEIA underpowered-not-negative chip). `71f79b1` corrected 10 more (B_PROUD staffing hours, CAST co-primary count, MATCH bleeding-endpoint mislabel on four surfaces, ENRICH putaminal-exclusion error on three record surfaces plus the ICH guide page, ATTEST-2 "interchangeable" overclaim, DEFUSE-3 common-OR gloss, ENCHANTED Study Mode pearl, plus BASICS and INSPIRES against their primary papers). Roughly 20 medium, 28 low and 4 cosmetic findings remain.
  - **Two systemic defects fixed alongside, both wider than any single trial.** (a) `DeltaBandChart` announced its band to screen readers as "N extra recoveries per 100 patients" on every chart regardless of endpoint polarity, so bad-outcome endpoints were inverted (ENRICH: 9 extra recoveries rather than 9 fewer deaths). Label is now polarity-neutral and names its endpoint; do not reintroduce polarity wording without a per-call-site prop. (b) The ICH guide page carried the same ENRICH eligibility error as the trial record, which is the fourth time a defect has been fixed in `trialData.ts` while a sibling copy survived in `guideContent.ts` or a pearl. Regression sweeps must run repo-wide on the retired string, not per-record.
  - **The audit's proposed fixes are not reliable.** Of the findings worked so far, the audit repeatedly located a real contradiction and then resolved it backwards, INSPIRES being the clearest: it treated a mis-transcribed "24 to 72 hours" eligibility item as authoritative and would have rewritten the one surface that was correct. Applied fixes must be derived from evidence, never copied from the `proposedFix` field.
  - **New evidence packet:** `docs/evidence-packets/2026-08-04-basics-inspires-window-and-lysis-rate.md` (BASICS IVT rate 79.5% not ~40%, PMID 34010530; INSPIRES randomised within 72 h with 12.8% inside 24 h, PMID 38157499).
  - **2026-08-04, high-risk band closed.** All 8 remaining high-clinical-risk findings are resolved. Four were already closed by earlier batches and the audit list was stale (NINDS hard-coded chart values, TASTE ITT-vs-per-protocol chart, ESCAPE-MeVO segment list, SKIP eligibility block). Four were fixed in `6275b31` and `bde0752`: THALES guideline attribution (see `docs/reviews/clinical-PR-thales-guideline-attribution.md`), RESCUE BT sICH significance, ICH blood-pressure recommendation strength, and an uncited Class I assertion for EVD in the ICH surgery synthesis. **38 findings remain: 4 medium, 28 low, 4 cosmetic, 1 needs-source.**
  - **The second-reviewer join was recovered** at `/tmp/wf/confirm.json`, keyed on `auditRef`. It was previously believed lost in a truncated workflow return. Of the STILL_PRESENT findings it covers, it records `fixIsCorrect` per finding, which is the field that flags the audit's wrong proposals. Use it before applying any remaining `proposedFix`.
  - **2026-08-05, trial-audit tail CLOSED.** The last 34 findings were run through independent verification plus an adversarial refute pass (workflow `wf_e4772eb4-9eb`, 12 agents) rather than applied from the list. Result: 27 packages applied unaltered, 4 refuted-then-salvaged, 2 cases where **the repo was correct and the audit was wrong** (BEST-MSU primaryDesign, ENRICH), 1 left open for lack of a source (RAISE: needs the full text for the absolute mRS 0-1 difference with interval and efficacy-set denominator). The audit's own `proposedFix` was wrong in **6 of 34**. Commit `2a132e1`.
  - **The refute stage paid for itself three times.** It caught a verifier asserting "SELECT2 did not report a comparable any-ICH rate" (SELECT2 reported 74.9% vs 34.9%; the claim was inferred from the figure being absent from THIS repo, and it reversed the finding's polarity), a CREST-2 bottom line that dropped "routine" and "no longer" and so contradicted its own synthesis two sentences earlier, and a CHANCE-2 pearl generalising a China-specific device's 85-minute turnaround to "many comprehensive stroke centers". **Keep the refute stage on any future batch.**
  - **Two defects nobody had listed** were found by the skeptic and fixed: `guideContent.ts` asserted the ANGEL-ASPECT sICH difference was statistically significant (it is P=0.12) on a rendered page, and a Study Mode pearl gave P=0.07 for the same endpoint the trial record puts at P=0.12.
  - **DeltaBandChart significance-colour bug, wider than any trial.** `pSig` was `parseFloat(pValue) < 0.05`; `parseFloat("<0.001")` is NaN, so every chart passing a "<" p-value painted a highly significant result in the not-significant grey. Three live call sites. Fixed by reusing `pEstablishesDifference`, which already handles the leading "<". Do not reintroduce a bare `parseFloat` here.
  - **Regression source closed.** `docs/2026_07_01/audit-findings.json` asserted the wrong EXTEND-IA day-3 p-value (P<0.001, which belongs to the 24-hour reperfusion co-primary; the published value is P=0.002, Campbell NEJM 2015, PMID 25671797). Corrected in place with a dated note, because a future audit run off that document would have reintroduced the defect it took a live publication fetch to remove. **Treat every historical audit doc as a potential regression source, not just a record.**
  - **2026-08-05, CITATION PROVENANCE AUDIT. Six registered citations had `quoted_text` transcribed from `src/data/aha2026StrokeGuideline.ts` rather than from the publication**, so they were validating our internal copy against a copy of itself. Their own comments say so ("Verbatim from src/data/aha2026StrokeGuideline.ts ..."). These sit inside the count the mirror check reports as "verified against a source", so that number has been materially overstated. Found by grepping the registry for comments that point at a repo path instead of a publication: `grep -n "Verbatim from src/\|from src/data\|per src/\|mirrors src/" src/lib/citations/registry.ts`. **Run that grep before trusting any coverage number.**
    - `aha-asa-2026-6.3` (supratentorial surgery) — FIXED. Both grades were wrong (LOE B-R for A; COR 2a for 2b).
    - `aha-asa-2026-4.7.4` (endovascular techniques) — FIXED. Wrong LOE (A for B-R) plus editorial text inside the verbatim field. Notable: here the CITATION was wrong and the mirror was right.
    - `aha-asa-2026-4.5` (blood glucose) — FIXED. Grades correct; a COR 2a was written in 2b language and the COR 3 was softened.
    - `aha-asa-2026-2.5` (mobile stroke units) — FIXED. **Two of four recommendations absent and BOTH present LOEs wrong** (rec 1 is A not B-R; rec 4 is B-NR not B-R). Mirror completed to all four.
    - `aha-asa-2026-4.7.1` (EVT concomitant with IVT) — FIXED. Both grades were RIGHT; text carried editorial emphasis and a line lifted from the Synopsis. Self-referential provenance does not always imply a wrong grade.
    - `aha-asa-2026-4.6.1` (thrombolysis decision-making, 7+ recommendations) and `aha-asa-2026-4.7.2` (large-core anterior EVT) — **STILL SELF-REFERENTIAL AND UNVERIFIED.** §4.6.1 spans several PDF pages with 11+ rows and needs its own pass. Spot-checked the mild-non-disabling COR 3 / LOE B-R row: grades correct. Everything else in those two is unchecked.
  - **2026-08-05, PATHWAY GUIDELINE-STRENGTH AUDIT (the audit V approved).** 188 Class/COR/Level assertions across 17 bedside and study surfaces, each checked against the published PDF rather than against the mirror, then adversarially refuted (workflow `wf_771e9746-e14`, 12 agents). Commit `d74c1a1`.
    - **83 CORRECT · 46 NEEDS_SOURCE · 59 proposed defects, of which the skeptic REFUTED 26 (44%).** Refutation reasons: overreach-beyond-evidence 11, misread-guideline-table 6, belongs-to-another-guideline 4, wrong-section 4, missed-sibling 1. **Never run this pass without the refute stage.** The audit-then-refute join in the workflow return was empty (`refutedCount: 0`) because the skeptic paraphrases the statement it echoes; the real join was reconstructed from `journal.jsonl` by pairing audit and refute agents positionally, verified by checking that each refute verdict names a file the paired slice owns. **If a future run reports 0 refuted, do not believe it: rebuild the join from the journal.**
    - **12 of the 16 wrong-strength findings were UPGRADES.** The repo systematically overstates how firmly the guideline recommends things. That is the direction that converts a discretionary option into an instruction.
    - **Worst finding: an inverted harm recommendation on the stroke-code checklist.** The PT/OT order rationale said mobilization within 24 h "improves functional outcomes and reduces complications", badged Class I Level A. AHA/ASA 2026 §5.7 rec 3 is **COR 3: Harm, LOE B-R** ("not recommended ... and may be harmful", AVERT). Highest-consequence surface in the product, strongest possible badge, opposite of the guideline.
    - **Second worst: the 3 to 4.5 h exclusion list** (age >80, NIHSS >25, any anticoagulant, prior stroke plus diabetes) presented as 2026 guideline policy at Class I Level B. Those are ECASS III trial entry criteria. §4.6.2 rec 1 is one undifferentiated 0 to 4.5 h population at COR 1 LOE A. As written it would have clinicians withhold thrombolysis from patients the guideline recommends treating at its strongest grade.
    - **46 assertions are governed by guidelines this repo does not hold** (2022 ICH, 2021 secondary prevention, lipids). That is a quarter of the bedside strength claims, unverifiable today. It is the single strongest argument for obtaining those documents.
### trial-primary-source-verification — Class E-clinical [ACTIVE]
- **Status:** wave 1 of ~6 done (`83fd10b`), wave 2 running. **This is the first pass that checks trial numbers against the PUBLISHED PAPER.** Every earlier round, including the whole 182-finding audit, only tested whether a record agreed with ITSELF, which a number that is wrong on six surfaces passes perfectly. That is why the audit kept looking finished while defects kept surfacing.
- **Baseline coverage when this started:** 117 trials; **62 had ZERO statistic traced to any source**, 55 of those on public question pages, 19 of those without even a PMID. 114 of 117 had been *named* in some audit artifact, which is not the same as verified and should never again be reported as if it were.
- **Wave 1 result: 18 trials, 500 quantities traced, ONE clean (TRACE-III).** 14 DEFECTS_FOUND, 3 PARTIALLY_UNRESOLVED. 29 findings with edits, 2 refuted, 27 applied. Defect rate on records nobody had flagged is comparable to the flagged ones, which retires the idea that the audit list bounded the problem.
- **Two agents were misled by `src/data/aha2026StrokeGuideline.ts`** when checking a class of recommendation, in a wave whose own rules warned about it. SELECT2 and ANGEL-ASPECT had misattributed §4.7.2 in OPPOSITE directions; I re-read the PDF rather than accept either agent's mirror-derived answer. **Wave 2 onward the schema has a `guideline-claim` kind and the skeptic refutes with `trusted-the-internal-mirror`; agents are forbidden from proposing a grade themselves.**
- **JOIN BUG, nearly shipped a refuted edit.** I paired verify and refute agents in journal-completion order rather than by slice, so refutations landed on the wrong findings. One already-applied edit turned out to be refuted and was reverted (the proposed ECASS III rewrite would have promoted the trial's SECONDARY global endpoint above its primary on a bedside page). Caught only because a skeptic's corrected edit named a different trial than the finding it was attached to. **Re-derived by unique problem-count per slice.** From wave 2 the refute schema carries a `trialId` checksum per verdict plus a `sliceTrials` list, and the script blocks any verdict whose trialId does not match the finding it indexes. **Never apply a join you have not validated.**
- **Acceptance:** every trial reaches ALL_CONFIRMED or has its unresolvable quantities listed; each wave carries its refute pass; gates + Gate 6.
- **PROGRESS: 105 of 117 trials verified against their primary publications.** Waves 1-6 complete: `83fd10b` (18 trials, 500 quantities, 1 clean), `c32e5cc` (18, 496, 2 clean), `046cb8c` (15 genuine, 587, 2 clean), `5c89f1f` (18, 470, 1 clean), `b17a5e7` (18, 446, 0 clean), `0570012` (18, 547, 1 clean). **~3,050 quantities traced; roughly 9 in 10 trials carried at least one defect.** All question-featured trials are covered.
- **WAVE 7 (FINAL 12) NOT RUN: session limit.** All 5 verify agents failed with "You've hit your session limit". Slices are staged and ready at `scratchpad/wf12/` (blocks + slice0-3.json). Remaining trials: **profess, racecat, triage-stroke, prost, prost-2, ims-iii, synthesis-expansion, mr-rescue, match, charisma, ist, cast**. None is currently surfaced on a question page. Re-run with the wave-4 script and `{"scratch": ".../wf12", "wave": 7}`; if the scratchpad has been cleaned, re-split the blocks with the snippet used for wave 6.
- **Defect classes this pass found that no earlier round could.** (a) **Wrong identifiers**: ANNEXA-I, DECIMAL and DESTINY each cited a PMID resolving to an unrelated paper, with correct-looking journal/year/authors around it. Invisible to every check and to a careful reader. (b) **Omission of the null half**: PICSS charted a favourable non-prespecified subgroup while the trial's own published comparison, pointing the other way, appeared nowhere in the repo; DECIMAL/DESTINY/HAMLET showed the pooled mortality benefit and omitted the pooled functional outcome whose interval touches zero. Nothing was *wrong*, so nothing flagged it. (c) **Fabricated eligibility gates**: DISTAL carried an ASPECTS exclusion the trial never had, alongside its own correct verbatim registry block. (d) **Ordinal-shift glosses**: three separate trials described a common/generalized OR as a per-category or one-step quantity.
- **Direction of error.** Overwhelmingly overstatement, but not exclusively: ORIGINAL invented a formulation caveat that would make a clinician DISCOUNT applicable evidence, and CHANCE understated a COR 1 LOE A recommendation as Class IIa. Do not assume the bias runs one way.
- **Three verification-MECHANISM failures, all caught by reconciliation rather than by a green result.** A stalled run (agents reading whole files), a misaligned verify/refute join (paired by completion order, not slice) that nearly shipped a refuted edit, and a cache replay that silently substituted a previous wave's trials. Mitigations now in the script: pre-split per-trial blocks, a `trialId` checksum on every refute verdict, and an `assignmentCheck` reconciling returned trials against assigned. Waves 4-6 came back clean on both. **Always reconcile outputs against expectations; never trust a green summary.**

  - **Sibling-surface failure recurred twice more.** The hemicraniectomy COR 2a error was fixed in the mirror in the morning and still survived on two pathway pages; a guide-page heading still read "Why AHA/ASA 2026 Rates This COR 3: No Benefit" about THALES after the verdict lines and comparison table were corrected. **Always finish with a repo-wide grep on the retired string, then re-grep the built output.**
  - **2026-08-05, GUIDELINE DOCUMENTS SUPPLIED BY V.** The 2022 AHA/ASA Spontaneous ICH guideline, the 2026 ACC/AHA Dyslipidemia guideline and the 2024 AHA/ASA PRIMARY Prevention guideline are now in `Articles/Guidelines/`. Extracted text lives in the session scratchpad; re-extract with `pdftotext -layout` if needed. Commit `839ac30`.
    - **ICH (20 claims): settled.** Corrections shipped: the ICH protocol step claimed Class I / Level A and told clinicians to avoid SBP below **110** when the guideline harm floor is **130** (COR 3: Harm, LOE B-R), a 20-point error in the permissive direction; TXA was badged Class III / Level A and called NOT recommended when General Hemostatic Treatments rec 2 is **COR 2b, LOE B-R, "effectiveness not well established"**, which is uncertainty rather than prohibition; STICH was badged Level B where the uncertain-benefit row is **LOE A**, and the second row (COR 2b, C-LD, craniotomy as a lifesaving measure in a deteriorating patient) was absent; the PATCH guideline line dropped the surgical carve-out (**COR 3: Harm applies only to patients NOT scheduled for emergency surgery; COR 2b, C-LD says transfusion might be considered for those who require emergency neurosurgery**). Anticoagulant reversal checked and needed no change (recs 1 to 3 are COR 1).
    - **I OVER-CORRECTED THE CEREBELLAR THRESHOLD ON 2026-08-04 AND FIXED IT TODAY.** The old page gated surgery on diameter >3 cm. I removed it and asserted the guideline sets no size threshold and size alone is never an indication. Wrong. Craniotomy for Posterior Fossa Hemorrhage rec 1 lists three ALTERNATIVE triggers and one is **volume ≥15 mL**, sufficient on its own. It is a volume, not a diameter. Lesson: removing an unsourced threshold is not the same as establishing that no threshold exists.
    - **Lipid (14 claims): ALL CORRECT AS WRITTEN, no edits.** Verified against §4.2.6 Secondary ASCVD Prevention: rec 4 COR 1 LOE A (high-intensity statin, LDL-C <55 for very high risk), rec 5 COR 1 LOE A (ezetimibe and/or a PCSK9 mAb, which is what makes "ezetimibe no longer required first" true), rec 6 COR 2a LOE B-R (bempedoic acid), rec 7 COR 2a LOE B-R (inclisiran); Measurement of Lp(a) rec 1 COR 1 LOE B-NR; and the statin-after-ICH claim against the ICH guideline at COR 2b LOE B-NR. `PostStrokeLipidManagement.tsx` is the best-sourced clinical surface in the repo. **Do not re-audit it without new evidence.**
    - **Secondary prevention (7 claims): RESOLVED 2026-08-05, commit `6f72136`.** V supplied the 2021 AHA/ASA secondary prevention guideline (Kleindorfer, Stroke 2021;52:e364-e467); registered in CLAUDE.md §13.9. **Every one of the 7 was badged Class I / Level B and the guideline grades all 7 as 2a.** Diagnostic Evaluation rec 6 (COR 2a, LOE B-R) covers echocardiography with or without contrast in cryptogenic stroke; rec 7 (COR 2a, LOE B-R) covers long-term rhythm monitoring in cryptogenic stroke WITHOUT a contraindication to anticoagulation, names modalities rather than a duration, and its supporting text states that improvement in patient outcomes with long-term rhythm monitoring **has not been established** — so our "minimum 24h (ideally 72h)" and "30 days is the standard" were both inventions. Glucose rec 5 (COR 2a, **LOE C-EO**) makes HbA1c screening reasonable and PREFERS HbA1c over fasting glucose, so the combined order is now HbA1c alone. **With this, every bedside strength claim in the repo has a held source.**
    - **The 2021 guideline carries a published correction** (Stroke 2021;52(7):e483-e484, doi 10.1161/STR.0000000000000383) that is NOT in the held PDF. Check it before relying on any single recommendation; an unread erratum already blocks a trial page here.
    - **2019 AIS guideline (4 claims): DOCUMENT NOT NEEDED, request withdrawn.** These attribute contraindication criteria (age >80, INR >1.7, therapeutic LMWH within 24 h) to a current guideline. Since 2026 supersedes 2019, the fix is to stop attributing them to a current guideline, not to cite a retired one. Open as a re-attribution task, not a sourcing one.
  - **Workflow `wf_73f78db4-f4e` FAILED COMPLETELY: all 5 agents stalled, 0 results, 683k tokens burned.** Cause is almost certainly that agents were pointed at multi-thousand-line extracted guideline text files and tried to read them whole. **If retried, pre-split the guideline text into per-section files and forbid whole-file reads.** The ICH and lipid work above was then done serially by the orchestrator with targeted grep plus single-page `pdftotext`, which is the method that has worked all session.
  - **Method that works for the PDF.** The whole-document text dump interleaves the two columns, so a grade cell can sit several lines from its recommendation. Never assign a grade from proximity in that dump. Re-extract the single page in reading order instead: `pdftotext -f <page> -l <page> "<pdf>" /tmp/page.txt`. That is what settled the §4.7.4 LOE dispute, where the dump made B-R look like it belonged to a neighbouring row.
  - **~~Still blocked on a source we do not hold~~ RESOLVED 2026-08-05.** The 2022 ICH guideline is now held (§13.9). The conservative 2-surfaces-against-1 call made on 2026-08-04 (Class IIb / Level B-R, harm floor <130) was **verified correct** against §Acute BP Lowering rec 3 (COR 2b, LOE B-R, target 140, goal range 130 to 150) and rec 5 (COR 3: Harm, LOE B-R, below 130 potentially harmful). Superseded by the 2026-08-05 ICH entry above.
...
- **2026-09-09 — Headache clinic ICHD-3 facial-pain expansion (13.2.1 GPN + 13.12 PIFP) + TN/ON source corrections** — commit 0f671e8. Class E, full chain: evidence packet (HIGH) -> architect plan review (approve-with-conditions, docs/reviews/arch-PLAN-headache-facial-pain-expansion-2026-09-08.md) -> medical-scientist spec -> clinical pre-gate (approve-with-conditions C1-C17, docs/reviews/clinical-PLAN-...) -> implementation -> clinical post-gate (approve-with-conditions P1/P2, both met; docs/reviews/clinical-PR-headache-facial-pain-expansion-2026-09-08.md) -> ui-architect ready. Gates: tsc clean, 377 vitest, check:claims (5 new tags live), check:humanizer, check:routes 48, check:trials 104, build + prerender 183/183. Gate 6 live verify PASS @ 2026-09-09 09:43: /, /pathways/headache-clinic, /pathways/migraine-pathway, /guide/headache-workup all 200 with titles; served lazy chunks probed directly (ClinicHeadachePathwayV4-CpGHEs7p.js carries glossopharyngeal-neuralgia, persistent-idiopathic-facial-pain, gpn-territory; MigrainePathway-Ba5Ai8lo.js carries neither "Level A, FDA-approved" nor "p.298"). Rollback: single git revert of 0f671e8.

- **2026-09-08 — Headache clinic v4 routing + TN/ON management fix batch** — commit 43f085b. Triple-gated: medical-scientist verify -> clinical-reviewer R1 block (BC-1..12) -> R2 narrow block (BI-1..4) -> R3 APPROVE (docs/reviews/clinical-PR-headache-v4-routing-2026-09-07.md); system-architect approve-with-conditions, all 7 conditions addressed (docs/reviews/arch-PR-headache-v4-routing-2026-09-07.md); seo-specialist sign-off ready. Gates: tsc clean, 366 vitest, check:claims (new claimId surface live), check:humanizer, check:routes 48, build+prerender green. Gate 6 live verify PASS @ 2026-09-08 13:23: / 200; /pathways/headache-clinic 200 with new title (FIRST prerender of this route — sitemap gap fixed); /pathways/se-pathway 200 (PathwayHeader blast-radius page); /guide/headache-workup 200 (retargeted safety-strip link destination). Rollback: single git revert of 43f085b.
- [x] 2026-07-27 — Copy feedback now reaches screen readers: 10 live-region surfaces — Class C (2a62443 group A + 3d69f97 group B). Closes `toast-live-region-a11y`, opened off the mobile-first review of 468e864. V approved two commits (A then B) and assertive-on-failure. **Scope was 10 surfaces, not the 1 in the brief, and I had caused 6 of them**: the inline toasts added hours earlier by the clipboard sweep carried `role="status" aria-live="polite"` on conditionally-mounted nodes, which is the same defect as `CalculatorToast`. Root cause across all of them: a live region created only when a message fires. Screen readers reliably announce text changing INSIDE an existing region but frequently miss a region inserted into the accessibility tree and populated in the same commit (WCAG 2.1 SC 4.1.3; iOS VoiceOver is the worst offender). **The repo had already solved this once and not propagated it** — `PathwayCascadeNotice.tsx:82-86` carries an explicit "a11y A-01: aria-live region must be present from mount, not injected after the fact" note, and `LKWTimePicker.tsx:1193` says the same. New `src/components/a11y/LiveAnnouncer.tsx` generalises that pattern. Three design points are load-bearing and easy to undo by accident (all documented in the file): (1) both regions mounted for the page lifetime, empty when idle; (2) polite and assertive are SEPARATE nodes, never one node whose role is swapped, because some AT caches an element's role at first computation and never recomputes it; (3) a repeated identical message clears the region then re-sets it a tick later, because writing the same string is not a DOM mutation and announces nothing, which bit exactly when a clinician taps Copy a second time after the first attempt seemed to do nothing. **Group A (2a62443, 21 files)** migrated `CalculatorToast` (12 calculators), the Elan/EVT/Migraine toasts, the embedded stroke-scale widget, the cocktail summary, and the stroke-basics toast; success paths included, since announcing failures while staying silent on success is worse than consistent silence. Visual nodes became `aria-hidden` so nothing announces twice, and the visible bubble stayed CONDITIONAL on purpose: it carries background and padding, so mounting it permanently would have parked an empty dark pill on every calculator page (the naive reading of "always mount it", caught by the accessibility review before it shipped). **Group B (3d69f97, 7 files)** covered surfaces with NO announcement at all, strictly worse than group A but dependent on its primitive: the announcer went into `PathwayHeader` itself so all four consumers passing `copyConfirm` (ExtendedIVT, StatusEpilepticus, ClinicHeadache, PostStrokeLipid) were covered without touching their pages, plus `ThrombolysisEligibilityModal` and `CodeModeStep3/4` (the stroke-code EMR note). Favourites toasts on three pathways had ZERO ARIA and were folded into each page's existing announcer. **Also fixed a non-a11y bug found in passing**: `CalculatorToast` had no `pointer-events-none`, so a visible toast at `z-[60]` could swallow a tap meant for the control underneath it, affecting sighted users too. Verified against BUILT output, not just source: the polite and assertive regions are both present and empty in the idle prerendered DOM of a calculator page, and no empty toast bubble renders. QA both groups: 275 tests, tsc clean, build 173/173, claims/routes/trials/chains/card-meta/humanizer green; Gate 6 green on push. **Verification gap, unchanged from the clipboard work:** no React component test library exists (adding one is a dependency change needing its own approval), so render behaviour rests on typecheck + the build-output check, and actual screen-reader announcement is UNVERIFIED without a device — same open item as `nihss-copy-ios-device-qa`, and one VoiceOver pass on a real iPhone would close both. Gate 6 PASS on production, verified in the live DOM after unregistering the service worker (the SW was serving a cached page and initially made the fix look absent): both regions present and empty on `/calculators/glasgow-coma-scale`, no stray toast pill. **Side effect found during that verify, benign but worth knowing:** the live page carries FOUR empty regions, not two. Because the announcer is now always mounted, the prerender step bakes a pair into the static HTML, and since React owns only `#root` it never reclaims those body-level portal nodes, so it mounts a second pair on load. The prerendered pair stays permanently empty and is never announced, so announcements are unaffected; it is DOM cruft, not a defect. If it is ever worth removing, the cheapest route is rendering the announcer inline instead of portalling to `document.body` (portal was chosen so no `overflow:hidden` or `aria-hidden` ancestor can neutralise the region). Deferred, filed not fixed: three conditionally-mounted live regions on RESULTS panels (`ElanPathway:464`, `MigrainePathway:1476`, `StrokeBasicsWorkflowV2:433`) are a different surface and were left alone; the accessibility review also flagged a possible visual overlap between the toast and the expanded drawer at certain content heights, pre-existing and needing judgement against real content, so it is a `mobile-first-developer` follow-up rather than part of this change.
- [x] 2026-07-27 — Clipboard hardening sweep: 24 copy surfaces across 33 files — Class C (53a92dd phase 1 + 0810c8a phase 2). Follow-up to the NIHSS iOS fix (468e864); V approved two commits, riskiest first, plus a visible failure state on the icon-only surfaces. **The brief's own triage was wrong and grepping beat trusting the list.** Three surfaces logged as "safe but silently swallow" (`AscvdRiskCalculator:375`, `ClinicHeadachePathwayV4:144`, `PostStrokeLipidManagement:140`) used `navigator.clipboard?.writeText(...).catch(() => {})` with the confirm setter OUTSIDE the promise chain, so it fired regardless: same false-success defect as NIHSS, not a lesser one. Four further sites were absent from the brief entirely and surfaced only by grepping raw `navigator.clipboard`: `CodeModeStep3:206` + `CodeModeStep4:497` (the stroke-code EMR note, wrapped in try/catch that only `console.error`'d under `import.meta.env.DEV`, so a failed copy of the code note was invisible in production), `CaseDetailModal:61` (honest toast already, no legacy fallback), and `usePwaInstall:141`. Final tally: 9 lied, 4 threw with an unhandled rejection, 12 were correct but silent. **Phase 1 (53a92dd, 17 files)** fixed everything that lied or threw; **phase 2 (0810c8a, 16 files)** gave the silent ones a visible failure state. Mechanism: every site routes through `copyToClipboard` so the success indicator can only fire on a real copy and the sync-first `execCommand` fallback covers in-app WKWebViews. Feedback shapes: `PathwayHeader` gained a red 'failed' label (prop still accepts `boolean` so consumers could migrate across the two commits); icon-only surfaces (NihssCalculatorEmbed, PathwayCocktailSummary, ThrombolysisEligibilityModal, CodeModeStep3/4) gained a warning glyph + text, since an icon alone cannot explain a blocked clipboard; toast surfaces carry `{text, ok}` instead of a boolean and their nodes gained `role="status" aria-live="polite"`; `ProtocolModal` gained `onCopyFailure` threaded through its three wrappers. **Wording is surface-aware** and lives in `utils/clipboard.ts` (`COPY_FAILED_USE_SEND` / `COPY_FAILED_USE_BROWSER`): only ~15 surfaces actually have a Send button, so reusing the NIHSS wording everywhere would have pointed clinicians at a button that is not there. Also fixed dead state in `AscvdRiskCalculator` (`copyConfirm` was set but never rendered, so that calculator gave no copy feedback at all; now on the shared toast like its siblings). tsc caught six share-result handlers that also needed the new shape and now report `'failed'` instead of staying silent. **No raw `navigator.clipboard` call sites remain in app code.** Verification gap, stated plainly: the render tri-states have NO automated cover because the repo has no React component test library and adding one is a dependency change needing its own approval, so they rest on typecheck + live verify; and `clipboard.test.ts` still mocks `execCommand`, so the WebKit recipe remains unproven on real hardware (see `nihss-copy-ios-device-qa`, still open). Gates both phases: 275 tests, tsc clean, build 173/173, claims/routes/trials/chains/card-meta/humanizer green. Gate 6 PASS phase 1 (marker live in `PathwayHeader-CLdIzPdZ.js`). **Concurrency hazard hit during this task:** a second session was working in the same working tree (its audit landed as b37ad66) and was mid-edit on `trialData.ts`, `registry.ts`, `clinicalSynthesesByQuestion.ts`, `TrialPageNew.tsx` — substantive Class E clinical edits (changed PMIDs, rewritten `quoted_text`, altered trial statistics). Phase 1 was staged with `git add -u src/`, which would have swept those into a Class C commit had they landed a few minutes earlier; it missed them by timing luck alone. Phase 2 was staged by explicit path. **Lesson: `git add -u` is unsafe in this repo — CLAUDE.md §20 already says stage only the changed files, and this is why.** Both phases' gates ran against a tree containing that session's in-flight clinical edits, so they are not a clean isolation of the clipboard change.
- [x] 2026-07-27 — NIHSS copy silently failing on iPhone (user-reported) — Class C (commit 468e864). Root cause in `NihssCalculator.copyNihss`: `navigator.clipboard.writeText(buildText())` called with no optional chaining, no await and no catch, followed unconditionally by `showToast('Copied to clipboard')`. Minified to a comma-sequenced expression in the shipped bundle (`le=()=>{navigator.clipboard.writeText(oe()),f("Copied to clipboard",2e3)}`), confirming the toast was never chained to the promise. Two distinct failures: (1) in-app WKWebViews (LinkedIn/Facebook/Instagram) and non-secure contexts have no `navigator.clipboard`, so the call threw a synchronous TypeError and the toast never fired — a completely dead tap; (2) a WebKit `NotAllowedError` rejection left the success toast asserting a copy that never landed, with the PREVIOUS patient's note still on the clipboard, plus an unhandled rejection. Defect (2) is the clinically dangerous one: a false success on an EMR export. NIHSS was the outlier — 12 sibling calculators already routed through `src/utils/clipboard.ts`, and `CaseDetailModal.tsx:61` was already the gold standard (try/catch + explicit "Copy failed"). Fix: `copyToClipboard(text, onSuccess?, onFailure?)` gains an optional failure callback and a legacy `execCommand` fallback; when `navigator.clipboard.writeText` is absent the fallback runs SYNCHRONOUSLY before any await, because WebKit only honours execCommand while the user activation is live (awaiting a rejection spends it — this ordering is the whole fix and is asserted in the tests before the first await). `legacyCopy` uses an offscreen contentEditable+readOnly textarea with Range selection + setSelectionRange (plain `.select()` is a documented no-op on iOS), preserves and restores any prior user selection, never focuses, and cleans up in `finally`. **mobile-first-developer fresh-context review (§18 writer/reviewer) caught a hole in the fix itself:** the new Copy failure toast directs clinicians to Send, but `shareOrCopy.ts:53-59` still used raw `navigator.clipboard.writeText` (same blind spot) AND `onShareResult` had no `'failed'` branch, so a failed Send showed nothing at all — a clinician on the reported device could tap Copy (fails, toast), tap Send (fails, silent), and be left with no working path. Both fixed in the same commit: `shareOrCopy` routed through `copyToClipboard`, and a `'failed'` branch added ("Send failed. Open this page in your browser." — the in-app-browser escape hatch). 14 new tests across `clipboard.test.ts` + `shareOrCopy.test.ts`, jsdom per-file environment (`@vitest-environment jsdom`; jsdom was already a devDependency, repo default is `node`). QA: 275 tests, tsc clean, build exit 0 (173/173 prerendered), claims/routes/trials/chains/card-meta green. **check:humanizer BLOCKED the first commit attempt** on an em-dash in the new failure toast (§10.3 working as designed on authored UI copy, not just clinical prose); reworded to periods. Gate 6 PASS — verified against the deployed bundle (`NihssCalculator-6M0Vlae4.js`): both new toasts present, old false-success pattern absent, `execCommand` fallback shipped in `ShareButton-PIK3SPZW.js`. Recipe additionally probed under a real trusted click in Chromium (execCommand returned true, full selection, no scroll jump, no focus steal, node cleaned up). **Not verified on real WebKit** — the unit tests mock `execCommand` and `setSelectionRange` because jsdom implements neither, so "275 tests pass" proves call ordering, not that iOS honours this recipe; on-device QA (Safari + one in-app WebView) remains outstanding and is the one open risk. Follow-ups opened: `clipboard-hardening-sweep` and `toast-live-region-a11y` (see PENDING).
- [x] 2026-07-22 — ICAD Phase 1 follow-up: CASSISS + BASIS trial-page render blocks (V-flagged design defect) — Class C (commit 509373a). Root cause: TrialPageNew.tsx renders each trial via a dedicated per-trial block (`if (trialId === '<id>')`); CASSISS/BASIS had none, so they fell through to the generic fallback renderer, which omits the population/inclusion-exclusion, how-to-read-chart, how-to-interpret, safety, and trial-design sections. The trial data already carried every field; only the layout block was missing, so the pages rendered a stripped-down layout instead of the full SAMMPRIS design. Added dedicated archetype-A blocks mirroring SAMMPRIS: CASSISS neutral (DeltaBandChart winnerArm none, event-free 92.0 vs 92.8, HR 1.10, "NO BENEFIT" strip); BASIS positive (winnerArm treatment, event-free 95.6 vs 86.5, HR 0.32, amber "POSITIVE, WITH CAVEATS" strip preserving the balloon-not-stenting / single-country / composite-includes-revascularization / front-loaded-risk guardrails). Renderer-only, trial data unchanged. Verified both pages render the full SAMMPRIS section set (population, chart, study arms, how-to-read, how-to-interpret, safety, trial design, bedside pearl, see-also, bottom-line drawer) at 375px + desktop via the dev server, and 173/173 prerendered pages built with the sections present. tsc clean, build exit 0, humanizer/claims/routes/chains/card-meta green. Lesson: adding a trial to this repo requires a dedicated render block in TrialPageNew.tsx, not just the data record (and the sitemap entry per the separate prerender lesson).
- [x] 2026-07-22 — ICAD literature refresh Phase 1: CASSISS + BASIS trial cards, CASSISS PMID fix, ICAS-stenting synthesis rewrite — Class E (commits 5f35d1f + 4561456). Added CASSISS 2022 (archetype A, NEUTRAL, stenting HR 1.10, no benefit) and BASIS 2024 (archetype A, POSITIVE, first positive endovascular ICAS RCT: submaximal balloon angioplasty HR 0.32, 4.4% vs 13.5%) trial cards mirroring the SAMMPRIS archetype-A shape; wired into the icas-stenting question (trialCount 2→4) + catalogue (secondary-prevention) + regenerated cardmeta (110). Fixed gao-cassiss-2022 citation: PMID 35943471 (which resolved to the RESCUE-BT tirofiban trial) → 35943472, url→DOI, quoted_text corrected (N/endpoint/P=.82). Added citations sun-basis-2024, alexander-woven-2021, gutierrez-icas-review-2022; registered cassiss-primary-result + basis-primary-result (bedsidePearl surface). Rewrote the /trials/q/icas-stenting ClinicalSynthesisCard to fold in the BASIS 2024 bottom-line shift + WOVEN and Gutierrez 2022 as cited context; preserved AMM-first-line + AHA/ASA 2021 SP §5.5 anchor and the two-axis framing (stenting-as-initial-therapy fails per SAMMPRIS+CASSISS; submaximal balloon angioplasty benefits per BASIS, not a stenting endorsement). Evidence verified against PubMed (evidence-verifier + trial-statistician + orchestrator re-confirm of every PMID/DOI/NCT/stat; the CASSISS PMID error was a genuine wrong-trial mislink). Reviews: clinical approve (docs/reviews/clinical-PR-icad-refresh.md), architect approve-with-conditions/met (arch-PR-icad-refresh.md), rollback note (rollback-icad-refresh.md), evidence packet 2026-07-22-icad-refresh.md with numbered §8. Integrated on top of origin/main (SELECT2/RESCUE-BT chips + late-window IVT, 3 commits) via stash/ff/pop with no conflicts. WOVEN 2021 deferred to Phase 2 (single-arm data would render as a false two-bar comparison via the WEAVE-hardcoded archetype-G path; see PENDING woven-trial-card-phase2); retained as a cited reference in the synthesis prose. Sitemap fix 4561456 added /trials/{cassiss,basis}-trial so both prerender (173/173 routes, was 171) with correct titles/results. QA: tsc clean, build exit 0, claims/humanizer (0 em-dashes)/routes/chains/card-meta all green; local prerender confirmed CASSISS + BASIS detail pages carry correct titles and stats. Gate 6 live-verify pending post-deploy re-check of the two new detail routes (the question page was confirmed serving BASIS+CASSISS pre-integration).
- [x] 2026-07-19 — Late-window IVT: Path A territory-agnostic DWI wording + Path C LVO-first reorder + sICH bleeding caution — Class E (PR #8, squash b5becce). Path A DWI size question/options/result reworded so the WAKE-UP/THAWS <1/3-MCA threshold reads as a volume yardstick that applies to PCA/ACA strokes (threshold unchanged); Path C 9–24h reordered to ask LVO (ICA/M1/M2) before penumbra with an up-front LVO clarifier, and the non-LVO verdict now states a qualifying occlusion alone is not sufficient (penumbra still required); shared qualitative sICH caution (data-claim extended-ivt-sich-caution) on every eligible verdict + Path-C-only TRACE-III ~3% vs <1% line (data-claim trace-iii-late-tnk-sich). Two claims registered; TRACE-III quoted_text numeric substantiation + review_window_months:3 on aha-asa-2026-4.6.3. Reviews: architect approve-with-conditions (adopted), clinical approve-with-conditions (both conditions met) — docs/reviews/{arch,clinical}-extivt-pathc-reorder-sich-caution.md; evidence packet docs/evidence-packets/2026-07-19-extended-late-window-ivt-sich-caution.md. QA: tsc clean · 261/261 tests · claims/humanizer/routes pass · build exit 0 (168/171 prerendered; the 3 timeouts are heavy hub routes hitting the sandbox 30s prerender limit, not this diff). Verified by driving the dev server through Path A + Path C to an eligible verdict. Gate 6 live-verify not run from the sandbox (egress 403 to production); confirm on prod after deploy. Follow-ups: 3-month override on trace-iii-trial-2024; re-review §4.6.3/TRACE-III when primary NEJM full text + HOPE (JAMA 2025) are retrievable; generalize the Path C "beyond 9h" non-LVO wording for the unknown-onset/wake-up sub-route.
- [x] 2026-07-02 — App-wide em-dash / AI-style cleanup + humanizer-gate tightening — Class C-clinical. Fable hunt (workflow wf_878d2a1d) across all user-facing text surfaces found 620 fixable em-dashes (the trial/question DATA files were already clean — em-dashes there were all in code comments; the density was in rendering/UI, SEO metadata, guide, pathways, calculators). 33 verbatim-quote em-dashes (registry.ts quoted_text) correctly excluded as no-touch; only 10 non-em-dash AI tells (prose otherwise clean). Remediation shipped in gated batches, each change deterministically verified as a PURE punctuation swap (removed/added alphanumerics byte-identical after stripping punctuation — no word/number/drug/dose/threshold/CI/grade altered): (1) clinical surfaces, 248 em-dashes across 23 files — trial pages, pathways, calculators, guide (0bdb4ba; clinical-reviewer gate approve, docs/reviews/clinical-emdash-cleanup-2026-07-02.md; caught+reverted 2 fixer overreaches: a title-casing sweep on 17 pathway option-labels and one inserted word); (2) SEO metadata + billing calc, 105 em-dashes across 3 files (eaf0133; CMS verbatim quote preserved). (3) Gate tightening: check-humanizer.mjs widened 8→27 TARGETS, added block-comment stripping + multi-line-template extraction (closes the guideContent gap) + an allowlist for legitimate em-dashes (EvtPathway criteriaName switch-keys, CMS quote, signature bylines); cleaned the last two template sources (routeMeta.ts SEO titles — source of "DAWN — …" page titles; format.ts case export). (4) Wave 2 — 110 em-dashes across 42 more files the accurate glob-scan surfaced beyond wave 1: guide disease-pages (IvTpa, IchManagement, StrokeBasics, MS, MG, GBS, Meningitis, Seizure/Weakness/AMS/Vertigo workups, StatusEpilepticus, Thrombectomy, guideline mindmap), the authoritative guideline mirror aha2026StrokeGuideline.ts (recommendation-text punctuation only, wording untouched), CHA2DS2-VASc/Boston/ROPE calculators, clinical data (gcsScoreData, calculators, trialChainRegistry, mindmap data), and shared components. The gate's TARGETS then became GLOB-based over all user-facing dirs (src/pages, components, data, seo, config, lib/cases; skips tests, dev, generated, and the verbatim-quote citations dir), and countEmDashes ignores quoted single-char placeholder glyphs. Total ~470 em-dashes removed; the whole user-facing app is em-dash-clean and the humanizer gate now guards it end-to-end (verified: 0 ERROR on the full app, negative-tested to catch a regression in any guide page; block comments stripped, multi-line templates handled, legit em-dashes allowlisted). Each wave was deterministically proven a pure-punctuation swap (no clinical content altered) + clinical-reviewer-gated; all Gate-6 green.
- [x] 2026-07-02 — EVT pathway late-window reframe — Class C-clinical (commit ea9f28b, docs/reviews/clinical-evt-late-window-reframe.md). Fixed the ASPECTS ≥6 late-window evidence-badge misattribution (COR 1 · DAWN/DEFUSE-3 → COR 1 · §4.7.2 Rec #2; the rec is guideline-based, not a perfusion-mismatch result) + detail/pearl; reframed the late-window imaging step so NCCT ASPECTS (Path A) and CT perfusion (Path B) render as co-equal cards instead of ASPECTS-primary + perfusion-buried-fallback (user-reported: most centers use CT perfusion late-window). Decision logic unchanged. 5-lens Fable audit (ui-architect/mobile/a11y/clinical-reviewer/medical-scientist) → BLOCK on a perfusion COR-1 overclaim → remediated (removed the ungrounded grade) → re-gate approve-with-conditions; a11y fixes (id/htmlFor, role=group, aria-live, contrast, touch targets). Parked (PARKING LOT 2026-07-02): ground/downgrade the pre-existing DAWN/DEFUSE-3 verdict COR-1 badges; claim-tag EvtPathway.tsx (file-wide gap). tsc/build 171/171/Gate-6 green.
