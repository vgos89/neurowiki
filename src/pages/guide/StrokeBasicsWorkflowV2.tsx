...
import { ArrowLeft, ExternalLink, Copy, Brain, Info, AlertTriangle, AlertCircle, InfoIcon, FlaskConical, Eye, FileText as FileTextIcon, Zap, BookOpen, ChevronDown, GraduationCap } from 'lucide-react';
...
/** Fallback Step 1 data when user hasn't filled Card 1 yet */
...
/** Fallback Step 2 data when user hasn't filled Card 2 yet */
...
/** Study mode pearls trigger button — compact, rendered above card content in study mode */
...
// Main content component (must be inside StrokeBasicsLayout)
const MainContent: React.FC<{
  ...
  step1ModalOpen, setStep1ModalOpen,
  step2ModalOpen, setStep2ModalOpen,
  step3ModalOpen, setStep3ModalOpen,
  step4ModalOpen, setStep4ModalOpen,
  thrombectomyModalOpen, setThrombectomyModalOpen,
  onThrombectomyRecommendation,
  ...
  step4Orders, setStep4Orders,
  nihssModalOpen, setNihssModalOpen,
  eligibilityModalOpen, setEligibilityModalOpen,
  doorTimePickerOpen, setDoorTimePickerOpen,
  nihssFromModal, setNihssFromModal,
  eligibilityResult, setEligibilityResult,
  toastMessage, setToastMessage,
  tpaReversalModalOpen, setTpaReversalModalOpen,
  orolingualEdemaModalOpen, setOrolingualEdemaModalOpen,
  hemorrhageProtocolModalOpen, setHemorrhageProtocolModalOpen,
  pearlsData,
}) => {
  ...
  // Suppress unused-variable warning for setTimerStartTime / setTimerRunning
  ...
  return (
    <Suspense fallback={null}>
      <>
        ...
        {/* Clinical Context Bar — appears after Card 1 is saved */}
        {step1Data && (
        ...
        <div id="card-content-panel" className="px-3 sm:px-6 mt-1 pb-2">
          ...
          {activeCard === 1 && (
            <div>
              {workflowMode === 'study' && (
                <StudyPearlsButton
                  count={pearls['step-1']?.deep?.length || 0}
                  onClick={() => setStep1ModalOpen(true)}
                />
              )}
              <CodeModeStep1
                onComplete={(data) => {
          ...
          {activeCard === 2 && (
            <div>
              {workflowMode === 'study' && (
                <StudyPearlsButton
                  count={pearls['step-2']?.deep?.length || 0}
                  onClick={() => setStep2ModalOpen(true)}
                />
              )}
              ...
              {workflowMode === 'study' && (
                <details className="mt-4 group rounded-lg border border-purple-200 bg-purple-50 overflow-hidden">
                  ...
                  <div className="px-4 pb-4 pt-1 space-y-3">
                    ...
                    <p className="text-sm text-purple-800 leading-relaxed">
                      <strong>Cortical Signs:</strong> Aphasia, neglect, gaze deviation, hemianopia, and apraxia strongly suggest LVO. Multiple cortical signs (≥2) have 89% specificity for M1/M2 occlusion. Even with mild NIHSS, presence of cortical signs warrants urgent CTA and interventional radiology activation.
                    </p>
                    <p className="text-sm text-purple-800 leading-relaxed">
        ...
        {/* Thrombectomy Recommendation Card — appears when EVT pathway assessment is completed */}
        {thrombectomyRecommendation && (
        ...
        {/* Inclusion/Exclusion Criteria Modal */}
        ...
        {/* tPA/TNK Reversal Protocol Modal */}
        ...
        {/* Orolingual Edema Protocol Modal */}
        ...
        {/* Hemorrhage Protocol Modal — opens when ICH detected is selected in Card 2 */}
        ...
        {/* Door time analogue clock picker */}
        {doorTimePickerOpen && (
...
export default function StrokeBasicsWorkflowV2() {
  const [workflowMode, setWorkflowMode] = useState<'code' | 'study'>('code');
