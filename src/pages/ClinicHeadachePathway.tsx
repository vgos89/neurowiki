  ...
  return (
    <div className="min-h-dvh bg-slate-50 pb-32">
      ...
      <main className="max-w-2xl mx-auto px-5 pt-6 pb-4">
        ...
        {/* ── DIAGNOSTIC STRIP (temporary, V-requested 2026-05-27) ────────
            Shows every chip the evaluator received and, for each ICHD-3
            phenotype, which criterion fired/failed and whether it was
            definitional. Surfaces *why* a selection produced no match.
            REMOVE ONCE TUNING IS COMPLETE.
            Renders only when step 4 is complete (i.e. after a real
            selection), to avoid noise on the empty initial state. */}
        {step4Complete && (
          <section aria-label="Diagnostic" className="mt-8 space-y-4">
            <h2 className="text-[10px] font-bold text-amber-600 uppercase tracking-widest">
              Diagnostic (temporary)
            </h2>
            <div className="rounded-xl border-2 border-amber-200 bg-amber-50/40 p-4 space-y-4">
              {/* What the evaluator received */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-amber-700 mb-1.5">
                  Chips passed to evaluator ({chips.size})
                </p>
                {chips.size === 0 ? (
                  <p className="text-[12px] text-slate-500 italic">no chips</p>
                ) : (
                  <div className="flex flex-wrap gap-1.5">
                    {Array.from(chips).sort().map((c) => (
                      <code key={c} className="text-[10px] px-1.5 py-0.5 rounded bg-white border border-amber-200 text-slate-700 font-mono">
                        {c}
                      </code>
                    ))}
                  </div>
              ...
              {/* Per-phenotype gating result */}
              <div>
                <p className="text-[10px] font-bold uppercase tracking-widest text-amber-700 mb-1.5">
                  Per-phenotype evaluation
                </p>
                <ul className="space-y-2.5">
                  {HEADACHE_PHENOTYPES.map((p) => {
                    // Replicate evaluator state for diagnostic — we cannot
                    // easily re-export internals so we recompute here.
                    const isGatedOut =
                      p.hiddenUntilTrial !== undefined && !chips.has(p.hiddenUntilTrial.gateChip);
                    const definitionalFails = p.criteria
                      .filter((c) => c.definitional && !c.evaluate(chips));
                    const totalCriteria = p.criteria.length;
                    const passedCriteria = p.criteria.filter((c) => c.evaluate(chips)).length;
                    const isDroppedByDefinitional = definitionalFails.length > 0;
                    const isContinuousSuppressed =
                      ['migraine-without-aura', 'migraine-with-aura', 'episodic-tth', 'cluster-headache', 'paroxysmal-hemicrania', 'sunct-suna'].includes(p.id) &&
                      chips.has('dur-continuous');
                    const status = isGatedOut
                      ? 'hidden (no indo trial)'
                      : isContinuousSuppressed
                        ? 'suppressed (continuous)'
                        : isDroppedByDefinitional
                          ? 'DROPPED — definitional fail'
                          : `${passedCriteria}/${totalCriteria} criteria passed`;
                    const statusColor = isGatedOut || isContinuousSuppressed
                      ? 'text-slate-400'
                      : isDroppedByDefinitional
                        ? 'text-red-700'
                        : passedCriteria === totalCriteria
                          ...
                          : 'text-slate-700';
                    return (
                      <li key={p.id} className="text-[11px] border-l-2 border-amber-300 pl-2.5">
                        <div className="flex items-baseline justify-between gap-2 flex-wrap">
                          <span className="font-semibold text-slate-900">{p.name}</span>
                          <span className={`text-[10px] font-mono ${statusColor}`}>{status}</span>
                        </div>
                        <div className="text-[10px] text-slate-500 font-mono">{p.ichd3Section}</div>
                        <ul className="mt-1 space-y-0.5">
                          {p.criteria.map((c) => {
                            const passed = c.evaluate(chips);
                            const isDef = c.definitional === true;
                            const icon = passed ? '✓' : '✗';
                            const colour = passed
                              ? 'text-emerald-700'
                              : isDef
                                ...
                                : 'text-slate-500';
                            return (
                              <li key={c.id} className={`text-[10.5px] ${colour}`}>
                                <span className="mr-1">{icon}</span>
                                <code className="font-mono">{c.id}</code>
                                {isDef && <span className="ml-1 text-[9px] uppercase tracking-wider">[def]</span>}
              ...
              <p className="text-[10px] text-amber-800 italic leading-relaxed pt-1 border-t border-amber-200">
                [def] = definitional criterion. If any [def] fails, the
                phenotype is dropped entirely (does not appear as partial/
                probable). This strip is temporary and will be removed once
                the picker is tuned.
              </p>
            </div>
          </section>
