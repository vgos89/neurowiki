...
export const SectionPearls: React.FC<SectionPearlsProps> = ({
 ...
 !isDeepLearning ? 'text-purple-700' : 'text-gray-500'
 }`}>
            Quick
          </span>
          <button
 ...
 isDeepLearning ? 'bg-purple-600' : 'bg-gray-300'
 }`}
            role="switch"
      ...
      <div className="space-y-3">
        {pearls.map((pearl) => {
          ...
          return (
            <div
              key={pearl.id}
              className="p-4 bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-all hover:scale-[1.01]"
            >
              {/* Pearl Header */}
              ...
              {pearl.link && (
                <Link
                  to={pearl.link}
                  className="inline-flex items-center gap-1.5 mt-3 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-xs font-semibold text-blue-600 rounded-md transition-colors"
                >
