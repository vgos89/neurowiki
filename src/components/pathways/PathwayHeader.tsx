...
export const PathwayHeader: React.FC<PathwayHeaderProps> = ({
  ...
  return (
    <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-slate-100 shadow-sm -mx-4 px-4 md:-mx-6 md:px-6">
      {/* On ExtendedIVT, StatusEpilepticus, ClinicHeadache and PostStrokeLipid
          this label swap is the ONLY copy feedback there is, and a label
          changing on a button the user just pressed is not reliably announced.
          Consumers that pass no copyConfirm announce nothing, so pathways with
          their own toast announcer do not double up. */}
      <LiveAnnouncer
        message={
          copyConfirm === 'failed'
            ? 'Copy failed'
        ...
        tone={copyConfirm === 'failed' ? 'assertive' : 'polite'}
      />
